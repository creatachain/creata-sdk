...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	err = types.ValidateGenesis(genesisState, creataapp.MakeTestEncodingConfig().TxConfig.TxJSONDecoder())
	require.Error(t, err)
}
...
func TestGenesisStateFromGenFile(t *testing.T) {
	...
	require.Equal(t, "1000nametoken,100000000fcta", bankGenesis.Balances[0].GetCoins().String())
	require.Equal(t, "creata106vrzv5xkheqhjm023pxcxlqmcjvuhtfyachz4", bankGenesis.Balances[0].GetAddress().String())
	require.Equal(t, "The native staking token of the Creata Hub.", bankGenesis.DenomMetadata[0].GetDescription())
	require.Equal(t, "ucta", bankGenesis.DenomMetadata[0].GetBase())
	require.Equal(t, "mcta", bankGenesis.DenomMetadata[0].GetDenomUnits()[1].GetDenom())
	require.Equal(t, []string{"millicta"}, bankGenesis.DenomMetadata[0].GetDenomUnits()[1].GetAliases())
