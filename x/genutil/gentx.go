		...
		func(bal bankexported.GenesisBalance) (stop bool) {
			...
			if accAddress.Equals(addr) {
				...
				if coins.AmountOf(bondDenom).GT(accCoins.AmountOf(bondDenom)) {
					err = fmt.Errorf(
						"account %s has a balance in genesis, but it only has %v%s available to ucta, not %v%s",
...
type deliverTxfn func(msm.RequestDeliverTx) msm.ResponseDeliverTx
...
func DeliverGenTxs(
	...
	for _, genTx := range genTxs {
		...
		res := deliverTx(msm.RequestDeliverTx{Tx: bz})
		if !res.IsOK() {
			panic(res.Log)
		}
	}
