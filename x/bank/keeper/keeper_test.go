...
import (
	...
	vesting "github.com/creatachain/creata-sdk/x/auth/vesting/types"
	"github.com/creatachain/creata-sdk/x/bank/keeper"
...
func (suite *IntegrationTestSuite) TestSendCoinsFromModuleToAccount_Blacklist() {
	...
	maccPerms := creataapp.GetMaccPerms()
...
func (suite *IntegrationTestSuite) TestMsgSendEvents() {
	...
	events := ctx.EventManager().MSMEvents()
	...
	event1.Attributes = append(
		event1.Attributes,
		msm.EventAttribute{Key: []byte(types.AttributeKeyRecipient), Value: []byte(addr2.String())},
	)
	event1.Attributes = append(
		event1.Attributes,
		msm.EventAttribute{Key: []byte(types.AttributeKeySender), Value: []byte(addr.String())},
	)
	event1.Attributes = append(
		event1.Attributes,
		msm.EventAttribute{Key: []byte(sdk.AttributeKeyAmount), Value: []byte(newCoins.String())},
	)
	event2 := sdk.Event{
	...
	suite.Require().Equal(msm.Event(event1), events[0])
	suite.Require().Equal(msm.Event(event2), events[1])
	...
	suite.Require().Equal(msm.Event(event1), events[2])
	suite.Require().Equal(msm.Event(event2), events[3])
}
...
func (suite *IntegrationTestSuite) TestMsgMultiSendEvents() {
	...
	event2.Attributes = append(
		event2.Attributes,
		msm.EventAttribute{Key: []byte(types.AttributeKeySender), Value: []byte(addr2.String())},
	)
	event3 := sdk.Event{
		Type:       types.EventTypeTransfer,
		Attributes: []msm.EventAttribute{},
	...
	event3.Attributes = append(
		event3.Attributes,
		msm.EventAttribute{Key: []byte(types.AttributeKeyRecipient), Value: []byte(addr3.String())},
	)
	event3.Attributes = append(
		event3.Attributes,
		msm.EventAttribute{Key: []byte(sdk.AttributeKeyAmount), Value: []byte(newCoins.String())})
	...
	event4.Attributes = append(
		event4.Attributes,
		msm.EventAttribute{Key: []byte(types.AttributeKeyRecipient), Value: []byte(addr4.String())},
	)
	event4.Attributes = append(
		event4.Attributes,
		msm.EventAttribute{Key: []byte(sdk.AttributeKeyAmount), Value: []byte(newCoins2.String())},
	)

	suite.Require().Equal(msm.Event(event1), events[1])
	suite.Require().Equal(msm.Event(event2), events[2])
	suite.Require().Equal(msm.Event(event3), events[3])
	suite.Require().Equal(msm.Event(event4), events[4])
}
...
func (suite *IntegrationTestSuite) TestSpendableCoins() {
	...
	origCoins := sdk.NewCoins(sdk.NewInt64Coin("ucta", 100))
	delCoins := sdk.NewCoins(sdk.NewInt64Coin("ucta", 50))
...
func (suite *IntegrationTestSuite) TestVestingAccountSend() {
	...
	origCoins := sdk.NewCoins(sdk.NewInt64Coin("ucta", 100))
	sendCoins := sdk.NewCoins(sdk.NewInt64Coin("ucta", 50))
...
func (suite *IntegrationTestSuite) TestPeriodicVestingAccountSend() {
	...
	periods := vesting.Periods{
		vesting.Period{Length: int64(12 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("ucta", 50)}},
		vesting.Period{Length: int64(6 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("ucta", 25)}},
		vesting.Period{Length: int64(6 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("ucta", 25)}},
	}
