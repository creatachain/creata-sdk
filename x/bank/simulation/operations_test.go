...
import (
	...
	"github.com/creatachain/creata-sdk/x/bank/simulation"
...
func (suite *SimTestSuite) TestWeightedOperations() {
	...
	expected := []struct {
		...
		{creataappparams.DefaultWeightMsgSend, types.ModuleName, types.TypeMsgSend},
		{creataappparams.DefaultWeightMsgMultiSend, types.ModuleName, types.TypeMsgMultiSend},
	}
...
func (suite *SimTestSuite) TestSimulateMsgSend() {
	...
	suite.Require().Equal("65337742fcta", msg.Amount.String())
	suite.Require().Equal("creata1ghekyjucln7y67ntx7cf27m9dpuxxemn4c8g4r", msg.FromAddress)
	suite.Require().Equal("creata1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7u4x9a0", msg.ToAddress)
...
func (suite *SimTestSuite) TestSimulateMsgMultiSend() {
	...
	suite.Require().Equal("creata1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7u4x9a0", msg.Inputs[1].Address)
	suite.Require().Equal("185121068fcta", msg.Inputs[1].Coins.String())
	suite.Require().Len(msg.Outputs, 2)
	suite.Require().Equal("creata1ghekyjucln7y67ntx7cf27m9dpuxxemn4c8g4r", msg.Outputs[1].Address)
	suite.Require().Equal("260469617fcta", msg.Outputs[1].Coins.String())
