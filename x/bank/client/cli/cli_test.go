...
import (
	...
	"github.com/creatachain/creata-sdk/x/bank/client/cli"
...
func (s *IntegrationTestSuite) TestGetCmdQueryDenomsMetadata() {
	...
	testCases := []struct {
		...
		},
		{
			...
			args: []string{
				fmt.Sprintf("--%s=1", flags.FlagHeight),
				fmt.Sprintf("--%s=%s", cli.FlagDenom, "ucta"),
				fmt.Sprintf("--%s=json", tmcli.OutputFlag),
			},
...
func (s *IntegrationTestSuite) TestNewSendTxCmd() {
	...
	testCases := []struct {
		...
		},
		{
			...
			false,
			&sdk.TxResponse{},
			sdkerrors.ErrInsufficientFee.MSMCode(),
		},
		{
			"not enough gas",
			val.Address,
...
func (s *IntegrationTestSuite) TestBankMsgService() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			&sdk.TxResponse{},
			0,
			"/creata.bank.v1beta1.Msg/Send", // indicates we are using ServiceMsg and not a regular Msg
		},
	}
