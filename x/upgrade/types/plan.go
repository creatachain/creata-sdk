...
func (p Plan) String() string {
  ...
  %s
  Info: %s.
  Upgraded ICP Client: %s`, p.Name, dueUp, p.Info, upgradedClientStr)
}
...
func (p Plan) ValidateBasic() error {
	...
	if p.Time.Unix() > 0 && p.UpgradedClientState != nil {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "ICP chain upgrades must only set height")
	}
...
// IsICPPlan will return true if plan includes ICP client information
func (p Plan) IsICPPlan() bool {
...
func (p Plan) UnpackInterfaces(unpacker codectypes.AnyUnpacker) error {
	...
	var clientState icpexported.ClientState
