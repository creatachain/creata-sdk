...
import (
	...
	"github.com/creatachain/creata-sdk/store/prefix"
...
func (k Keeper) ScheduleUpgrade(ctx sdk.Context, plan types.Plan) error {
	...
	// clear any old ICP state stored by previous plan
	oldPlan, exists := k.GetUpgradePlan(ctx)
	if exists && oldPlan.IsICPPlan() {
		k.ClearICPState(ctx, oldPlan.Height-1)
	}
...
func (k Keeper) SetUpgradedClient(ctx sdk.Context, planHeight int64, cs icpexported.ClientState) error {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetUpgradedClient(ctx sdk.Context, height int64) (icpexported.ClientState, error) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) SetUpgradedConsensusState(ctx sdk.Context, planHeight int64, cs icpexported.ConsensusState) error {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetUpgradedConsensusState(ctx sdk.Context, lastHeight int64) (icpexported.ConsensusState, error) {
	store := ctx.KVStore(k.storeKey)
...
// ClearICPState clears any planned ICP state
func (k Keeper) ClearICPState(ctx sdk.Context, lastHeight int64) {
	// delete ICP client and consensus state from store if this is ICP plan
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) ApplyUpgrade(ctx sdk.Context, plan types.Plan) {
	...
	// Must clear ICP state after upgrade is applied as it is stored separately from the upgrade plan.
	// This will prevent resubmission of upgrade msg after upgrade is already completed.
	if plan.IsICPPlan() {
		k.ClearICPState(ctx, plan.Height-1)
	}
