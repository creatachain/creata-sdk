...
func setupTest(height int64, skip map[int64]bool) TestSuite {
	db := dbm.NewMemDB()
	app := creataapp.NewCreataApp(log.NewNopLogger(), db, nil, true, skip, creataapp.DefaultNodeHome, 0, creataapp.MakeTestEncodingConfig(), creataapp.EmptyAppOptions{})
	genesisState := creataapp.NewDefaultGenesisState(app.AppCodec())
...
func VerifyDoICPLastBlock(t *testing.T) {
	...
	req := msm.RequestBeginBlock{Header: newCtx.BlockHeader()}
	s.module.BeginBlock(newCtx, req)

	// plan Height is at ctx.BlockHeight+1
	consState, err := s.keeper.GetUpgradedConsensusState(newCtx, s.ctx.BlockHeight()+1)
	require.NoError(t, err)
	require.Equal(t, &icptmtypes.ConsensusState{Timestamp: newCtx.BlockTime(), NextValidatorsHash: nextValsHash}, consState)
}

func VerifyDoICPUpgrade(t *testing.T) {
	t.Log("Verify that a panic happens at the upgrade time/height")
	newCtx := s.ctx.WithBlockHeight(s.ctx.BlockHeight() + 1).WithBlockTime(time.Now())

	// Check ICP state is set before upgrade using last height: s.ctx.BlockHeight()
	...
	require.NotNil(t, cs, "ICP client is nil before upgrade")
	...
	require.NotNil(t, consState, "ICP consensus state is nil before upgrade")
	...
	// Check ICP state is cleared after upgrade using last height: s.ctx.BlockHeight()
	cs, err = s.keeper.GetUpgradedClient(newCtx, s.ctx.BlockHeight())
	require.Error(t, err, "retrieved upgraded client after upgrade plan is applied")
	require.Nil(t, cs, "ICP client is not-nil after upgrade")

	consState, err = s.keeper.GetUpgradedConsensusState(newCtx, s.ctx.BlockHeight())
	require.Error(t, err, "retrieved upgraded consensus state after upgrade plan is applied")
	require.Nil(t, consState, "ICP consensus state is not-nil after upgrade")
}
...
func TestHaltIfTooNew(t *testing.T) {
	...
	req = msm.RequestBeginBlock{Header: futCtx.BlockHeader()}
	require.NotPanics(t, func() {
...
func VerifyCleared(t *testing.T, newCtx sdk.Context) {
	...
	bz, err := s.querier(newCtx, []string{types.QueryCurrent}, msm.RequestQuery{})
	require.NoError(t, err)
	require.Nil(t, bz)
}
...
func TestNoSpuriousUpgrades(t *testing.T) {
	...
	req := msm.RequestBeginBlock{Header: s.ctx.BlockHeader()}
	require.NotPanics(t, func() {
...
func TestPlanStringer(t *testing.T) {
  ...
  Info: .
  Upgraded ICP Client: no upgraded client provided`, types.Plan{Name: "test", Time: ti}.String())
	require.Equal(t, `Upgrade Plan
  Name: test
  Height: 100
  Info: .
  Upgraded ICP Client: no upgraded client provided`, types.Plan{Name: "test", Height: 100}.String())
	require.Equal(t, fmt.Sprintf(`Upgrade Plan
  Name: test
  Height: 100
  Info: .
  Upgraded ICP Client: %s`, clientState), types.Plan{Name: "test", Height: 100, UpgradedClientState: cs}.String())
}
...
func TestICPUpgradeWithoutSkip(t *testing.T) {
	s := setupTest(10, map[int64]bool{})
	cs, err := clienttypes.PackClientState(&icptmtypes.ClientState{})
	require.NoError(t, err)
