...
func TestSetWithdrawAddr(t *testing.T) {
	...
	addr := creataapp.AddTestAddrs(app, ctx, 2, sdk.NewInt(1000000000))
...
func TestWithdrawValidatorCommission(t *testing.T) {
	...
	valCommission := sdk.DecCoins{
		...
		sdk.NewDecCoinFromDec("ucta", sdk.NewDec(3).Quo(sdk.NewDec(2))),
	}

	addr := creataapp.AddTestAddrs(app, ctx, 1, sdk.NewInt(1000000000))
	valAddrs := creataapp.ConvertAddrsToValAddrs(addr)

	// set module account coins
	distrAcc := app.DistrKeeper.GetDistributionAccount(ctx)
	err := app.BankKeeper.SetBalances(ctx, distrAcc.GetAddress(), sdk.NewCoins(
		sdk.NewCoin("mytoken", sdk.NewInt(2)),
		sdk.NewCoin("ucta", sdk.NewInt(2)),
	))
	...
	expCoins := sdk.NewCoins(sdk.NewCoin("ucta", expTokens))
	...
	require.Equal(t, sdk.NewCoins(
		...
		sdk.NewCoin("ucta", expTokens.AddRaw(1)),
	), balance)
	...
	require.Equal(t, sdk.DecCoins{
		...
		sdk.NewDecCoinFromDec("ucta", sdk.NewDec(1).Quo(sdk.NewDec(2))),
	}, remainder)
...
func TestFundCommunityPool(t *testing.T) {
	...
	amount := sdk.NewCoins(sdk.NewInt64Coin("ucta", 100))
