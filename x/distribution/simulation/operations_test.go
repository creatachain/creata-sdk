...
func (suite *SimTestSuite) TestWeightedOperations() {
	...
	expected := []struct {
		...
		{creataappparams.DefaultWeightMsgSetWithdrawAddress, types.ModuleName, types.TypeMsgSetWithdrawAddress},
		{creataappparams.DefaultWeightMsgWithdrawDelegationReward, types.ModuleName, types.TypeMsgWithdrawDelegatorReward},
		{creataappparams.DefaultWeightMsgWithdrawValidatorCommission, types.ModuleName, types.TypeMsgWithdrawValidatorCommission},
		{creataappparams.DefaultWeightMsgFundCommunityPool, types.ModuleName, types.TypeMsgFundCommunityPool},
	}
...
func (suite *SimTestSuite) TestSimulateMsgSetWithdrawAddress() {
	...
	// begin a new block
	suite.app.BeginBlock(msm.RequestBeginBlock{Header: tmproto.Header{Height: suite.app.LastBlockHeight() + 1, AppHash: suite.app.LastCommitID().Hash}})
	...
	suite.Require().Equal("creata1ghekyjucln7y67ntx7cf27m9dpuxxemn4c8g4r", msg.DelegatorAddress)
	suite.Require().Equal("creata1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7u4x9a0", msg.WithdrawAddress)
...
func (suite *SimTestSuite) TestSimulateMsgWithdrawDelegatorReward() {
	...
	suite.Require().True(operationMsg.OK)
	suite.Require().Equal("creatavaloper1l4s054098kk9hmr5753c6k3m2kw65h686d3mhr", msg.ValidatorAddress)
	suite.Require().Equal("creata1d6u7zhjwmsucs678d7qn95uqajd4ucl9jcjt26", msg.DelegatorAddress)
...
func (suite *SimTestSuite) testSimulateMsgWithdrawValidatorCommission(tokenName string) {
	...
	valCommission := sdk.NewDecCoins(
		...
		sdk.NewDecCoinFromDec("ucta", sdk.NewDec(1).Quo(sdk.NewDec(1))),
	)
	...
	suite.Require().True(operationMsg.OK)
	suite.Require().Equal("creatavaloper1tnh2q55v8wyygtt9srz5safamzdengsn9dsd7z", msg.ValidatorAddress)
...
func (suite *SimTestSuite) TestSimulateMsgFundCommunityPool() {
	...
	suite.Require().True(operationMsg.OK)
	suite.Require().Equal("4896096fcta", msg.Amount.String())
	suite.Require().Equal("creata1ghekyjucln7y67ntx7cf27m9dpuxxemn4c8g4r", msg.Depositor)
