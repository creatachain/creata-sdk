...
func NewTestStdFee() StdFee {
	return NewStdFee(100000,
		sdk.NewCoins(sdk.NewInt64Coin("cta", 150)),
	)
}
...
func TestStdSignBytes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fmt.Sprintf("{\"account_number\":\"3\",\"chain_id\":\"1234\",\"fee\":{\"amount\":[{\"amount\":\"150\",\"denom\":\"cta\"}],\"gas\":\"100000\"},\"memo\":\"memo\",\"msgs\":[[\"%s\"]],\"sequence\":\"6\",\"timeout_height\":\"10\"}", addr),
		},
		{
			args{"1234", 3, 6, 0, defaultFee, []sdk.Msg{testdata.NewTestMsg(addr)}, "memo"},
			fmt.Sprintf("{\"account_number\":\"3\",\"chain_id\":\"1234\",\"fee\":{\"amount\":[{\"amount\":\"150\",\"denom\":\"cta\"}],\"gas\":\"100000\"},\"memo\":\"memo\",\"msgs\":[[\"%s\"]],\"sequence\":\"6\"}", addr),
		},
	}
...
func TestTxValidateBasic(t *testing.T) {
	...
	require.Error(t, err)
	_, code, _ := sdkerrors.MSMInfo(err, false)
	require.Equal(t, sdkerrors.ErrInsufficientFee.MSMCode(), code)
	...
	require.Error(t, err)
	_, code, _ = sdkerrors.MSMInfo(err, false)
	require.Equal(t, sdkerrors.ErrNoSignatures.MSMCode(), code)
	...
	require.Error(t, err)
	_, code, _ = sdkerrors.MSMInfo(err, false)
	require.Equal(t, sdkerrors.ErrUnauthorized.MSMCode(), code)
	...
	require.Error(t, err)
	_, code, _ = sdkerrors.MSMInfo(err, false)
	require.Equal(t, sdkerrors.ErrInvalidRequest.MSMCode(), code)
...
func TestGetSignaturesV2(t *testing.T) {
	...
	fee := NewStdFee(50000, sdk.Coins{sdk.NewInt64Coin("cta", 150)})
	sig := StdSignature{PubKey: pubKey, Signature: dummy}
