...
import (
	...
	clienttx "github.com/creatachain/creata-sdk/client/tx"
...
func (s IntegrationTestSuite) TestSimulateTx_GRPCGateway() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			s.Require().NoError(err)
			res, err := rest.PostRequest(fmt.Sprintf("%s/creatachain/tx/v1beta1/simulate", val.APIAddress), "application/json", req)
			s.Require().NoError(err)
			if tc.expErr {
...
func (s IntegrationTestSuite) TestGetTxEvents_GRPCGateway() {
	...
	testCases := []struct {
		...
		{
			"empty params",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs", val.APIAddress),
			true,
			"must declare at least one event to search",
		},
		{
			"without pagination",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s", val.APIAddress, "message.action='send'"),
			false,
			"",
		},
		{
			"with pagination",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s&pagination.offset=%d&pagination.limit=%d", val.APIAddress, "message.action='send'", 0, 10),
			false,
			"",
		},
		{
			"valid request: order by asc",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s&events=%s&order_by=ORDER_BY_ASC", val.APIAddress, "message.action='send'", "message.module='bank'"),
			false,
			"",
		},
		{
			"valid request: order by desc",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s&events=%s&order_by=ORDER_BY_DESC", val.APIAddress, "message.action='send'", "message.module='bank'"),
			false,
			"",
		},
		{
			"invalid request: invalid order by",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s&events=%s&order_by=invalid_order", val.APIAddress, "message.action='send'", "message.module='bank'"),
			true,
			"is not a valid tx.OrderBy",
		},
		{
			"expect pass with multiple-events",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s&events=%s", val.APIAddress, "message.action='send'", "message.module='bank'"),
			false,
			"",
		},
		{
			"expect pass with escape event",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs?events=%s", val.APIAddress, "message.action%3D'send'"),
			false,
			"",
		},
	}
...
func (s IntegrationTestSuite) TestGetTx_GRPCGateway() {
	...
	testCases := []struct {
		...
		{
			"dummy hash",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs/%s", val.APIAddress, "deadbeef"),
			true, "tx (DEADBEEF) not found",
		},
		{
			"good hash",
			fmt.Sprintf("%s/creatachain/tx/v1beta1/txs/%s", val.APIAddress, s.txRes.TxHash),
			false, "",
		},
	}
	for _, tc := range testCases {
...
func (s IntegrationTestSuite) TestBroadcastTx_GRPCGateway() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			req, err := val.ClientCtx.JSONMarshaler.MarshalJSON(tc.req)
			s.Require().NoError(err)
			res, err := rest.PostRequest(fmt.Sprintf("%s/creatachain/tx/v1beta1/txs", val.APIAddress), "application/json", req)
			s.Require().NoError(err)
			if tc.expErr {
				s.Require().Contains(string(res), tc.expErrMsg)
			} else {
