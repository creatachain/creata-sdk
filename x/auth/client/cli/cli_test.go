...
func (s *IntegrationTestSuite) TestCLISignBatch() {
	...
	val.ClientCtx.HomeDir = strings.Replace(val.ClientCtx.HomeDir, "creatad", "simcli", 1)
...
func TestGetBroadcastCommand_OfflineFlag(t *testing.T) {
	...
	clientCtx = clientCtx.WithTxConfig(creataapp.MakeTestEncodingConfig().TxConfig)
...
func TestGetBroadcastCommand_WithoutOfflineFlag(t *testing.T) {
	...
	txCfg := creataapp.MakeTestEncodingConfig().TxConfig
	...
	from, err := sdk.AccAddressFromBech32("creata1cxlt8kznps92fwu3j6npahx4mjfutydyene2qw")
	require.NoError(t, err)
	to, err := sdk.AccAddressFromBech32("creata1cxlt8kznps92fwu3j6npahx4mjfutydyene2qw")
	require.NoError(t, err)
	err = builder.SetMsgs(banktypes.NewMsgSend(from, to, sdk.Coins{sdk.NewInt64Coin("ucta", 10000)}))
	require.NoError(t, err)
