...
import (
	...
	kmultisig "github.com/creatachain/creata-sdk/crypto/keys/multisig"
	...
	authcli "github.com/creatachain/creata-sdk/x/auth/client/cli"
	authrest "github.com/creatachain/creata-sdk/x/auth/client/rest"
	authtest "github.com/creatachain/creata-sdk/x/auth/client/testutil"
	"github.com/creatachain/creata-sdk/x/auth/legacy/legacytx"
	bankcli "github.com/creatachain/creata-sdk/x/bank/client/testutil"
	"github.com/creatachain/creata-sdk/x/bank/types"
	icpcli "github.com/creatachain/creata-sdk/x/icp/core/04-channel/client/cli"
	icpsolomachinecli "github.com/creatachain/creata-sdk/x/icp/light-clients/06-solomachine/client/cli"
)
...
// Create an ICP tx that's encoded as amino-JSON. Since we can't amino-marshal
// a tx with "creata-sdk/MsgTransfer" using the SDK, we just hardcode the tx

func mkICPStdTx() []byte {
	icpTx := `{
		"account_number": "68",
		"chain_id": "stargate-4",
		"fee": {
		...
		"msg": [
		  {
			"type": "creata-sdk/MsgTransfer",
			"value": {
			  "receiver": "creata1q9wtnlwdjrhwtcjmt2uq77jrgx7z3usrq2yz7z",
			  "sender": "creata1q9wtnlwdjrhwtcjmt2uq77jrgx7z3usrq2yz7z",
	...
	req := fmt.Sprintf(`{"tx":%s,"mode":"async"}`, icpTx)
...
func (s *IntegrationTestSuite) TestEncodeICPTx() {
	val := s.network.Validators[0]
...
func (s *IntegrationTestSuite) TestBroadcastICPTxRequest() {
	val := s.network.Validators[0]
...
// testQueryICPTx is a helper function to test querying txs which:
// - show an error message on legacy REST endpoints
// - succeed using gRPC
// In practice, we call this function on ICP txs.
func (s *IntegrationTestSuite) testQueryICPTx(txRes sdk.TxResponse, cmd *cobra.Command, args []string) {
	val := s.network.Validators[0]

	errMsg := "this transaction cannot be displayed via legacy REST endpoints, because it does not support" +
		" Amino serialization. Please either use CLI, gRPC, gRPC-gateway, or directly query the Augusteum RPC" +
		" endpoint to query this transaction. The new REST endpoint (via gRPC-gateway) is "

	// Test that legacy endpoint return the above error message on ICP txs.
	testCases := []struct {
		desc string
		url  string
	}{
	...
	grpcJSON, err := rest.GetRequest(fmt.Sprintf("%s/creatachain/tx/v1beta1/txs/%s", val.APIAddress, txRes.TxHash))
	s.Require().NoError(err)
...
// TestLegacyRestErrMessages creates two ICP txs, one that fails, one that
...
// signed with proto, since ICP won't support legacy amino at all we are
// considering a message from ICP module.
func (s *IntegrationTestSuite) TestLegacyRestErrMessages() {
	val := s.network.Validators[0]

	// Write consensus json to temp file, used for an ICP message.
	consensusJSON := testutil.WriteToNewTempFile(
		s.T(),
		`{"public_key":{"@type":"/creata.crypto.secp256k1.PubKey","key":"A/3SXL2ONYaOkxpdR5P8tHTlSlPv1AwQwSFxKRee5JQW"},"diversifier":"diversifier","timestamp":"10"}`,
	)
	...
	testCases := []struct {
		...
		{
			"Successful ICP message",
			icpsolomachinecli.NewCreateClientCmd(),
			[]string{
				"1",                  // dummy sequence
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			s.testQueryICPTx(txRes, tc.cmd, tc.args)
		})
	}
}
...
func (s *IntegrationTestSuite) TestLegacyMultisig() {
	...
	// Sign with account1
	val1.ClientCtx.HomeDir = strings.Replace(val1.ClientCtx.HomeDir, "creatad", "simcli", 1)
