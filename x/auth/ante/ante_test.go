...
func (suite *AnteTestSuite) TestAnteHandlerFees() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.BankKeeper.SetBalances(suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("cta", 149)))
			},
			false,
			false,
			sdkerrors.ErrInsufficientFunds,
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("cta"), sdk.NewInt(149)))

				suite.app.BankKeeper.SetBalances(suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("cta", 150)))
			},
			false,
			true,
			nil,
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, modAcc.GetAddress()).AmountOf("cta"), sdk.NewInt(150)))
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("cta"), sdk.NewInt(0)))
			},
			false,
			false,
			sdkerrors.ErrInsufficientFunds,
	...
	testCases := []TestCase{
		{
			...
			func() {
				feeAmount = sdk.NewCoins(sdk.NewInt64Coin("cta", 0))
				gasLimit = 0
			},
			false,
			false,
		...
		},
		{
			...
			func() {
				feeAmount = sdk.NewCoins(sdk.NewInt64Coin("cta", 0))
				gasLimit = 801
				suite.txBuilder.SetMemo("msmninasidniandsinasindiansdiansdinaisndiasndiadninsd")
			},
			false,
			false,
			sdkerrors.ErrOutOfGas,
