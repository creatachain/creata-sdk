...
func (suite *KeeperTestSuite) TestSetAndGetConnection() {
	...
	_, existed := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetConnection(suite.chainA.GetContext(), connA.ID)
...
func (suite *KeeperTestSuite) TestSetAndGetClientConnectionPaths() {
	...
	_, existed := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetClientConnectionPaths(suite.chainA.GetContext(), clientA)
	suite.False(existed)

	connections := []string{"connectionA", "connectionB"}
	suite.chainA.App.ICPKeeper.ConnectionKeeper.SetClientConnectionPaths(suite.chainA.GetContext(), clientA, connections)
	paths, existed := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetClientConnectionPaths(suite.chainA.GetContext(), clientA)
	suite.True(existed)
...
func (suite KeeperTestSuite) TestGetAllConnections() {
	clientA, clientB, connA0, connB0 := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
	...
	connections := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetAllConnections(suite.chainA.GetContext())
...
func (suite KeeperTestSuite) TestGetAllClientConnectionPaths() {
	clientA0, _, connA0, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
	clientA1, clientB1, connA1, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
	...
	connPaths := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetAllClientConnectionPaths(suite.chainA.GetContext())
...
func (suite *KeeperTestSuite) TestGetTimestampAtHeight() {
	...
	cases := []struct {
		...
		{"verification success", func() {
			_, _, connA, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
	...
	for _, tc := range cases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			actualTimestamp, err := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetTimestampAtHeight(
