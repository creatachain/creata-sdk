...
func (suite *KeeperTestSuite) TestConnOpenInit() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			connectionID, err := suite.chainA.App.ICPKeeper.ConnectionKeeper.ConnOpenInit(suite.chainA.GetContext(), clientA, counterparty, version, delayPeriod)
...
func (suite *KeeperTestSuite) TestConnOpenTry() {
	...
	testCases := []struct {
		...
		{"success with delay period", func() {
			...
			suite.chainA.App.ICPKeeper.ConnectionKeeper.SetConnection(suite.chainA.GetContext(), connA.ID, conn)

			// commit in order for proof to return correct value
			suite.coordinator.CommitBlock(suite.chainA)
			suite.coordinator.UpdateClient(suite.chainB, suite.chainA, clientB, exported.Augusteum)
		...
		{"invalid counterparty client", func() {
			...
			tmClient, ok := counterpartyClient.(*icptmtypes.ClientState)
			suite.Require().True(ok)
			tmClient.ChainId = "wrongchainid"

			suite.chainA.App.ICPKeeper.ClientKeeper.SetClientState(suite.chainA.GetContext(), clientA, tmClient)
		}, false},
		...
		{"consensus state verification failed", func() {
			...
			consState, found := suite.chainA.App.ICPKeeper.ClientKeeper.GetLatestClientConsensusState(suite.chainA.GetContext(), clientA)
			suite.Require().True(found)

			tmConsState, ok := consState.(*icptmtypes.ConsensusState)
			suite.Require().True(ok)

			tmConsState.Timestamp = time.Now()
			suite.chainA.App.ICPKeeper.ClientKeeper.SetClientConsensusState(suite.chainA.GetContext(), clientA, counterpartyClient.GetLatestHeight(), tmConsState)
		...
		{"invalid previous connection is in TRYOPEN", func() {
			...
			err = suite.coordinator.UpdateClient(suite.chainB, suite.chainA, clientB, exported.Augusteum)
			suite.Require().NoError(err)
		...
		{"invalid previous connection has invalid versions", func() {
			...
			connection, found := suite.chainB.App.ICPKeeper.ConnectionKeeper.GetConnection(suite.chainB.GetContext(), connB.ID)
			suite.Require().True(found)

			connection.State = types.INIT
			connection.Versions = []*types.Version{{}}

			suite.chainB.App.ICPKeeper.ConnectionKeeper.SetConnection(suite.chainB.GetContext(), connB.ID, connection)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			connectionID, err := suite.chainB.App.ICPKeeper.ConnectionKeeper.ConnOpenTry(
...
func (suite *KeeperTestSuite) TestConnOpenAck() {
	...
	testCases := []struct {
		...
		{"invalid counterparty client", func() {
			...
			suite.chainB.App.ICPKeeper.ClientKeeper.SetClientState(suite.chainB.GetContext(), clientB, tmClient)
		...
		{"invalid counterparty connection ID", func() {
			...
			connection, found := suite.chainA.App.ICPKeeper.ConnectionKeeper.GetConnection(suite.chainA.GetContext(), connA.ID)
			suite.Require().True(found)
		...
		{"incompatible ICP versions", func() {
			clientA, clientB = suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
			connA, connB, err := suite.coordinator.ConnOpenInit(suite.chainA, suite.chainB, clientA, clientB)
			suite.Require().NoError(err)
		...
		{"feature set verification failed - unsupported feature", func() {
			...
			version = types.NewVersion(types.DefaultICPVersionIdentifier, []string{"ORDER_ORDERED", "ORDER_UNORDERED", "ORDER_DAG"})
		}, false},
		{"self consensus state not found", func() {
			clientA, clientB = suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
			connA, connB, err := suite.coordinator.ConnOpenInit(suite.chainA, suite.chainB, clientA, clientB)
		...
		{"consensus state verification failed", func() {
			...
			consState, found := suite.chainB.App.ICPKeeper.ClientKeeper.GetLatestClientConsensusState(suite.chainB.GetContext(), clientB)
			suite.Require().True(found)
			...
			tmConsState.Timestamp = time.Now()
			suite.chainB.App.ICPKeeper.ClientKeeper.SetClientConsensusState(suite.chainB.GetContext(), clientB, counterpartyClient.GetLatestHeight(), tmConsState)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			err := suite.chainA.App.ICPKeeper.ConnectionKeeper.ConnOpenAck(
...
func (suite *KeeperTestSuite) TestConnOpenConfirm() {
	...
	testCases := []struct {
		...
		{"chain B's connection state is not TRYOPEN", func() {
			...
			clientA, clientB, _, _ = suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
		}, false},
		{"connection state verification failed", func() {
			// chainA is in INIT
			clientA, clientB = suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			err := suite.chainB.App.ICPKeeper.ConnectionKeeper.ConnOpenConfirm(
