...
func (suite *KeeperTestSuite) TestQueryConnection() {
	...
	testCases := []struct {
		...
		{"connection not found",
			func() {
				req = &types.QueryConnectionRequest{
					ConnectionId: icptesting.InvalidID,
				}
			},
			false,
		},
		{
			...
			func() {
				...
				suite.chainA.App.ICPKeeper.ConnectionKeeper.SetConnection(suite.chainA.GetContext(), connA.ID, expConnection)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				clientA1, clientB1, connA1, connB1 := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
...
func (suite *KeeperTestSuite) TestQueryClientConnections() {
	...
	testCases := []struct {
		...
		},
		{
			"success",
			func() {
				clientA, clientB, connA0, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
				connA1, _ := suite.coordinator.CreateConnection(suite.chainA, suite.chainB, clientA, clientB)
				expPaths = []string{connA0.ID, connA1.ID}
				suite.chainA.App.ICPKeeper.ConnectionKeeper.SetClientConnectionPaths(suite.chainA.GetContext(), clientA, expPaths)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.chainA.App.ICPKeeper.ConnectionKeeper.SetConnection(suite.chainA.GetContext(), connA.ID, types.ConnectionEnd{})
		...
		},
		{
			"success",
			func() {
				clientA, _, connA, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
