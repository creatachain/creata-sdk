...
func (suite *KeeperTestSuite) TestVerifyClientState() {
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			_, clientB, connA, _ := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
			...
			if tc.malleateCounterparty {
				tmClient, _ := counterpartyClient.(*icptmtypes.ClientState)
			...
			err := suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyClientState(
...
func (suite *KeeperTestSuite) TestVerifyClientConsensusState() {
	...
	cases := []struct {
		...
		{"verification failed", func() {
			...
			suite.chainB.App.ICPKeeper.ClientKeeper.SetClientConsensusState(suite.chainB.GetContext(), clientB, clientState.GetLatestHeight(), tmConsState)
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			proofHeight := clienttypes.NewHeight(0, uint64(suite.chainB.GetContext().BlockHeight()-1))
			consensusState, found := suite.chainA.App.ICPKeeper.ClientKeeper.GetSelfConsensusState(suite.chainA.GetContext(), consensusHeight)
			suite.Require().True(found)

			err := suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyClientConsensusState(
				suite.chainA.GetContext(), connection,
...
func (suite *KeeperTestSuite) TestVerifyConnectionState() {
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			err := suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyConnectionState(
				suite.chainA.GetContext(), connection,
...
func (suite *KeeperTestSuite) TestVerifyChannelState() {
	...
	for _, tc := range cases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			err := suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyChannelState(
...
func (suite *KeeperTestSuite) TestVerifyPacketCommitment() {
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			packet := channeltypes.NewPacket(icptesting.TestHash, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, defaultTimeoutHeight, 0)
			...
			if tc.changePacketCommitmentState {
				packet.Data = []byte(icptesting.InvalidID)
			}

			commitment := channeltypes.CommitPacket(suite.chainB.App.ICPKeeper.Codec(), packet)
			err = suite.chainB.App.ICPKeeper.ConnectionKeeper.VerifyPacketCommitment(
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			if tc.changeAcknowledgement {
				ack = []byte(icptesting.InvalidID)
			}

			err = suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyPacketAcknowledgement(
				suite.chainA.GetContext(), connection, malleateHeight(proofHeight, tc.heightDiff), proof,
...
func (suite *KeeperTestSuite) TestVerifyPacketReceiptAbsence() {
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			err = suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyPacketReceiptAbsence(
				suite.chainA.GetContext(), connection, malleateHeight(proofHeight, tc.heightDiff), proof,
...
func (suite *KeeperTestSuite) TestVerifyNextSequenceRecv() {
	...
	for _, tc := range cases {
		...
		suite.Run(tc.msg, func() {
			...
			err = suite.chainA.App.ICPKeeper.ConnectionKeeper.VerifyNextSequenceRecv(
				suite.chainA.GetContext(), connection, malleateHeight(proofHeight, tc.heightDiff), proof,
