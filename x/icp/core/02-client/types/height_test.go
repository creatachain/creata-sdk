...
func TestParseChainID(t *testing.T) {
	cases := []struct {
		...
		{"creatamainnet-3", 3, true},
		{"a-1", 1, true},
		{"creata-mainnet-40", 40, true},
		{"creatamainnet-3-39", 39, true},
		{"creatamainnet--", 0, false},
		{"creatamainnet-03", 0, false},
		{"creatamainnet--4", 0, false},
		{"creatamainnet-3.4", 0, false},
		{"creatamainnet", 0, false},
		{"a--1", 0, false},
		{"-1", 0, false},
		{"--1", 0, false},
...
func TestSetRevisionNumber(t *testing.T) {
	...
	chainID, err := types.SetRevisionNumber("creatamainnet", 3)
	...
	chainID = "creatamainnet-3"

	chainID, err = types.SetRevisionNumber(chainID, 4)
	require.NoError(t, err, "valid revision format failed SetRevisionNumber")
	require.Equal(t, "creatamainnet-4", chainID, "valid revision format returned incorrect string on SetRevisionNumber")
}
...
func (suite *TypesTestSuite) TestSelfHeight() {
	...
	ctx = ctx.WithChainID("creatamainnet")
	...
	ctx = ctx.WithChainID("creatamainnet-3")
