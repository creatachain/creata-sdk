...
func (suite *TypesTestSuite) TestPackClientState() {
	...
	testCases := []struct {
		...
		{
			"solo machine client",
			icptesting.NewSolomachine(suite.T(), suite.chainA.Codec, "solomachine", "", 2).ClientState(),
			true,
		},
		{
			"augusteum client",
			icptmtypes.NewClientState(chainID, icptesting.DefaultTrustLevel, icptesting.TrustingPeriod, icptesting.UnbondingPeriod, icptesting.MaxClockDrift, clientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, false, false),
			true,
		},
		{
			"localhost client",
...
func (suite *TypesTestSuite) TestPackConsensusState() {
	testCases := []struct {
	...
	}{
		{
			...
			icptesting.NewSolomachine(suite.T(), suite.chainA.Codec, "solomachine", "", 2).ConsensusState(),
			true,
		},
		{
			"augusteum consensus",
...
func (suite *TypesTestSuite) TestPackHeader() {
	testCases := []struct {
		...
		{
			"solo machine header",
			icptesting.NewSolomachine(suite.T(), suite.chainA.Codec, "solomachine", "", 2).CreateHeader(),
			true,
		},
		{
			"augusteum header",
...
func (suite *TypesTestSuite) TestPackMisbehaviour() {
	testCases := []struct {
	...
	}{
		{
			...
			icptesting.NewSolomachine(suite.T(), suite.chainA.Codec, "solomachine", "", 2).CreateMisbehaviour(),
			true,
		},
		{
			"augusteum misbehaviour",
			icptmtypes.NewMisbehaviour("augusteum", suite.chainA.LastHeader, suite.chainA.LastHeader),
			true,
		},
		{
			"nil",
