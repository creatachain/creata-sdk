...
func (suite *KeeperTestSuite) TestQueryClientStates() {
	...
	testCases := []struct {
		...
		},
		{
			"success",
			func() {
				clientA1, _ := suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
				clientA2, _ := suite.coordinator.CreateClient(suite.chainA, suite.chainB, exported.Augusteum)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				cs := icptmtypes.NewConsensusState(
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				cs2 := icptmtypes.NewConsensusState(
				...
				clientState := icptmtypes.NewClientState(
					testChainID, icptmtypes.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, testClientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, false, false,
				)
