...
func (suite *KeeperTestSuite) TestCreateClient() {
	cases := []struct {
		...
		{"success", icptmtypes.NewClientState(testChainID, icptmtypes.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, testClientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, false, false), true},
...
func (suite *KeeperTestSuite) TestUpdateClientAugusteum() {
	// Must create header creation functions since suite.header gets recreated on each test case
	createFutureUpdateFn := func(s *KeeperTestSuite) *icptmtypes.Header {
	...
	createPastUpdateFn := func(s *KeeperTestSuite) *icptmtypes.Header {
	...
	var (
		updateHeader *icptmtypes.Header
	...
	cases := []struct {
		...
		{"valid update", func() error {
			...
			intermediateConsState := &icptmtypes.ConsensusState{
		...
		{"valid past update", func() error {
			...
			prevConsState := &icptmtypes.ConsensusState{
		...
		{"frozen client before update", func() error {
			clientState = &icptmtypes.ClientState{FrozenHeight: types.NewHeight(0, 1), LatestHeight: testClientHeight}
			suite.keeper.SetClientState(suite.ctx, testClientID, clientState)
			updateHeader = createFutureUpdateFn(suite)
	...
	for i, tc := range cases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expPass {
				...
				expConsensusState := &icptmtypes.ConsensusState{
...
func (suite *KeeperTestSuite) TestUpdateClientLocalhost() {
	...
	err := suite.chainA.App.ICPKeeper.ClientKeeper.UpdateClient(ctx, exported.Localhost, nil)
	suite.Require().NoError(err)

	clientState, found := suite.chainA.App.ICPKeeper.ClientKeeper.GetClientState(ctx, exported.Localhost)
...
func (suite *KeeperTestSuite) TestUpgradeClient() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				upgradedClient = icptmtypes.NewClientState("newChainId", icptmtypes.DefaultTrustLevel, trustingPeriod, ubdPeriod+trustingPeriod, maxClockDrift, newClientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, false, false)
		...
		},
		{
			...
			setup: func() {
				...
				tmClient, ok := cs.(*icptmtypes.ClientState)
		...
		{
			name: "augusteum client VerifyUpgrade fails",
			setup: func() {
				...
				upgradedClient = icptmtypes.NewClientState("wrongchainID", icptmtypes.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, newClientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, true, true)
	...
	for _, tc := range testCases {
		...
		err := suite.chainA.App.ICPKeeper.ClientKeeper.UpgradeClient(suite.chainA.GetContext(), clientA, upgradedClient, upgradedConsState, proofUpgradedClient, proofUpgradedConsState)
...
func (suite *KeeperTestSuite) TestCheckMisbehaviourAndUpdateState() {
	...
	altPrivVal := icptestingmock.NewPV()
	...
	bothSigners := icptesting.CreateSortedSignerArray(altPrivVal, suite.privVal, altVal, suiteVal)
	...
	testCases := []struct {
		name         string
		misbehaviour *icptmtypes.Misbehaviour
		malleate     func() error
		expPass      bool
	}{
		{
			"trusting period misbehavior should pass",
			&icptmtypes.Misbehaviour{
...
func (suite *KeeperTestSuite) TestUpdateClientEventEmission() {
	clientID, _ := suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
