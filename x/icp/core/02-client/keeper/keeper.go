...
func (k Keeper) ValidateSelfClient(ctx sdk.Context, clientState exported.ClientState) error {
	tmClient, ok := clientState.(*icptmtypes.ClientState)
	if !ok {
		return sdkerrors.Wrapf(types.ErrInvalidClient, "client must be a Augusteum client, expected: %T, got: %T",
			&icptmtypes.ClientState{}, tmClient)
	}
	...
	if err := light.ValidateTrustLevel(tmClient.TrustLevel.ToAugusteum()); err != nil {
	...
	if len(tmClient.UpgradePath) != 0 {
		// For now, SDK ICP implementation assumes that upgrade path (if defined) is defined by SDK upgrade module
		expectedUpgradePath := []string{upgradetypes.StoreKey, upgradetypes.KeyUpgradedICPState}
...
func (k Keeper) IterateClients(ctx sdk.Context, cb func(clientID string, cs exported.ClientState) bool) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		// key is icp/{clientid}/clientState
