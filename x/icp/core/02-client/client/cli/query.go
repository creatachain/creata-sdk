...
import (
	...
	"github.com/creatachain/creata-sdk/x/icp/core/02-client/client/utils"
...
func GetCmdQueryHeader() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:    "Query the latest Augusteum header of the running chain",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			header, height, err := utils.QueryAugusteumHeader(clientCtx)
			if err != nil {
				return err
			}
...
func GetCmdParams() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "params",
		Short:   "Query the current icp client parameters",
		Long:    "Query the current icp client parameters",
		Args:    cobra.NoArgs,
