...
import (
	...
	clientutils "github.com/creatachain/creata-sdk/x/icp/core/02-client/client/utils"
...
// If prove is true, it performs an MSM store query in order to retrieve the merkle proof. Otherwise,
// it uses the gRPC query client.
func QueryChannel(
	clientCtx client.Context, portID, channelID string, prove bool,
) (*types.QueryChannelResponse, error) {
	if prove {
		return queryChannelMSM(clientCtx, portID, channelID)
	}
...
func queryChannelMSM(clientCtx client.Context, portID, channelID string) (*types.QueryChannelResponse, error) {
...
// prove is true, it performs an MSM store query in order to retrieve the
...
func QueryChannelClientState(
	...
	if prove {
		clientStateRes, err := clientutils.QueryClientStateMSM(clientCtx, res.IdentifiedClientState.ClientId)
		if err != nil {
			return nil, err
		}

		// use client state returned from MSM query in case query height differs
...
func QueryChannelConsensusState(
	...
	if prove {
		consensusStateRes, err := clientutils.QueryConsensusStateMSM(clientCtx, res.ClientId, height)
		if err != nil {
			return nil, err
		}
...
func QueryNextSequenceReceive(
	...
	if prove {
		return queryNextSequenceRecvMSM(clientCtx, portID, channelID)
	}
...
func queryNextSequenceRecvMSM(clientCtx client.Context, portID, channelID string) (*types.QueryNextSequenceReceiveResponse, error) {
...
func QueryPacketCommitment(
	...
	if prove {
		return queryPacketCommitmentMSM(clientCtx, portID, channelID, sequence)
	}
...
func QueryPacketReceipt(
	...
	if prove {
		return queryPacketReceiptMSM(clientCtx, portID, channelID, sequence)
	}
...
func QueryPacketAcknowledgement(clientCtx client.Context, portID, channelID string, sequence uint64, prove bool) (*types.QueryPacketAcknowledgementResponse, error) {
	if prove {
		return queryPacketAcknowledgementMSM(clientCtx, portID, channelID, sequence)
	}
...
func queryPacketAcknowledgementMSM(clientCtx client.Context, portID, channelID string, sequence uint64) (*types.QueryPacketAcknowledgementResponse, error) {
