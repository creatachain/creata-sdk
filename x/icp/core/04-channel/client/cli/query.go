...
import (
	...
	"github.com/creatachain/creata-sdk/x/icp/core/04-channel/client/utils"
...
func GetCmdQueryChannel() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Query an ICP channel end from a port and channel identifiers",
		Example: fmt.Sprintf(
...
func GetCmdQueryChannelClientState() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query icp channel client-state [port-id] [channel-id]", version.AppName),
		Args:    cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
