...
func (suite *KeeperTestSuite) TestSetChannel() {
	...
	channelA := suite.chainA.NextTestChannel(connA, icptesting.MockPort)
	_, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetChannel(suite.chainA.GetContext(), channelA.PortID, channelA.ID)
	suite.False(found)

	// init channel
	channelA, channelB, err := suite.coordinator.ChanOpenInit(suite.chainA, suite.chainB, connA, connB, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
	suite.NoError(err)

	storedChannel, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetChannel(suite.chainA.GetContext(), channelA.PortID, channelA.ID)
...
func (suite KeeperTestSuite) TestGetAllChannels() {
	...
	testchannel2, _, err := suite.coordinator.ChanOpenInit(suite.chainA, suite.chainB, connA1, connB1, icptesting.MockPort, icptesting.MockPort, types.UNORDERED)
	...
	channels := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllChannels(ctxA)
...
func (suite KeeperTestSuite) TestGetAllSequences() {
	...
	for _, seq := range []types.PacketSequence{seq1, seq2, seq3} {
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceSend(ctxA, seq.PortId, seq.ChannelId, seq.Sequence)
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceRecv(ctxA, seq.PortId, seq.ChannelId, seq.Sequence)
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceAck(ctxA, seq.PortId, seq.ChannelId, seq.Sequence)
	}

	sendSeqs := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketSendSeqs(ctxA)
	recvSeqs := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketRecvSeqs(ctxA)
	ackSeqs := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketAckSeqs(ctxA)
...
func (suite KeeperTestSuite) TestGetAllPacketState() {
	...
	for _, ack := range []types.PacketState{ack1, ack2, ack2dup, ack3} {
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketAcknowledgement(ctxA, ack.PortId, ack.ChannelId, ack.Sequence, ack.Data)
	}

	// set packet receipts
	for _, rec := range expReceipts {
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketReceipt(ctxA, rec.PortId, rec.ChannelId, rec.Sequence)
	}

	// set packet commitments
	for _, comm := range expCommitments {
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(ctxA, comm.PortId, comm.ChannelId, comm.Sequence, comm.Data)
	}

	acks := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketAcks(ctxA)
	receipts := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketReceipts(ctxA)
	commitments := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketCommitments(ctxA)
...
func (suite *KeeperTestSuite) TestSetSequence() {
	...
	seq, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceSend(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
	suite.Equal(one, seq)

	// initialized channel has next seq recv of 1
	seq, found = suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceRecv(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
	suite.Equal(one, seq)

	// initialized channel has next seq ack of
	seq, found = suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceAck(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
	suite.Equal(one, seq)

	nextSeqSend, nextSeqRecv, nextSeqAck := uint64(10), uint64(10), uint64(10)
	suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceSend(ctxA, channelA.PortID, channelA.ID, nextSeqSend)
	suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceRecv(ctxA, channelA.PortID, channelA.ID, nextSeqRecv)
	suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceAck(ctxA, channelA.PortID, channelA.ID, nextSeqAck)

	storedNextSeqSend, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceSend(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
	suite.Equal(nextSeqSend, storedNextSeqSend)

	storedNextSeqRecv, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceSend(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
	suite.Equal(nextSeqRecv, storedNextSeqRecv)

	storedNextSeqAck, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetNextSequenceAck(ctxA, channelA.PortID, channelA.ID)
	suite.True(found)
...
func (suite *KeeperTestSuite) TestGetAllPacketCommitmentsAtChannel() {
	...
	for i := uint64(1); i < seq; i++ {
		suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(ctxA, channelA.PortID, channelA.ID, i, hash)
		expectedSeqs[i] = true
	}
	...
	suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(ctxA, channelA1.PortID, channelA1.ID, maxSeq+1, hash)

	commitments := suite.chainA.App.ICPKeeper.ChannelKeeper.GetAllPacketCommitmentsAtChannel(ctxA, channelA.PortID, channelA.ID)
...
func (suite *KeeperTestSuite) TestSetPacketAcknowledgement() {
	...
	storedAckHash, found := suite.chainA.App.ICPKeeper.ChannelKeeper.GetPacketAcknowledgement(ctxA, channelA.PortID, channelA.ID, seq)
	suite.Require().False(found)
	suite.Require().Nil(storedAckHash)

	ackHash := []byte("ackhash")
	suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketAcknowledgement(ctxA, channelA.PortID, channelA.ID, seq, ackHash)

	storedAckHash, found = suite.chainA.App.ICPKeeper.ChannelKeeper.GetPacketAcknowledgement(ctxA, channelA.PortID, channelA.ID, seq)
	suite.Require().True(found)
	suite.Require().Equal(ackHash, storedAckHash)
	suite.Require().True(suite.chainA.App.ICPKeeper.ChannelKeeper.HasPacketAcknowledgement(ctxA, channelA.PortID, channelA.ID, seq))
}
