...
func (suite *KeeperTestSuite) TestChanOpenInit() {
	var (
		connA    *icptesting.TestConnection
		connB    *icptesting.TestConnection
		features []string
	...
	testCases := []testCase{
		{"success", func() {
			...
			suite.chainA.CreatePortCapability(suite.chainA.NextTestChannel(connA, icptesting.MockPort).PortID)
			portCap = suite.chainA.GetPortCapability(suite.chainA.NextTestChannel(connA, icptesting.MockPort).PortID)
		}, true},
		...
		{"connection version not negotiated", func() {
			...
			suite.chainA.App.ICPKeeper.ConnectionKeeper.SetConnection(
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			for _, order := range []types.Order{types.UNORDERED, types.ORDERED} {
				...
				counterparty := types.NewCounterparty(connB.FirstOrNextTestChannel(icptesting.MockPort).PortID, connB.FirstOrNextTestChannel(icptesting.MockPort).ID)
				channelA := connA.FirstOrNextTestChannel(icptesting.MockPort)

				channelID, cap, err := suite.chainA.App.ICPKeeper.ChannelKeeper.ChanOpenInit(
				...
				if tc.expPass && orderSupported {
					...
					chanCap, ok := suite.chainA.App.ScopedICPKeeper.GetCapability(
						suite.chainA.GetContext(),
...
func (suite *KeeperTestSuite) TestChanOpenTry() {
	...
	testCases := []testCase{
		{"success", func() {
			_, _, connA, connB = suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
			suite.coordinator.ChanOpenInit(suite.chainA, suite.chainB, connA, connB, icptesting.MockPort, icptesting.MockPort, types.ORDERED)

			suite.chainB.CreatePortCapability(suite.chainB.NextTestChannel(connB, icptesting.MockPort).PortID)
			portCap = suite.chainB.GetPortCapability(suite.chainB.NextTestChannel(connB, icptesting.MockPort).PortID)
		}, true},
		{"success with crossing hello", func() {
			_, _, connA, connB = suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
			_, channelB, err := suite.coordinator.ChanOpenInitOnBothChains(suite.chainA, suite.chainB, connA, connB, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
			suite.Require().NoError(err)
		...
		{"previous channel with invalid state", func() {
			...
			suite.coordinator.ChanOpenInit(suite.chainB, suite.chainA, connB, connA, icptesting.MockPort, icptesting.MockPort, types.UNORDERED)
		}, false},
		{"connection doesn't exist", func() {
			// any non-nil values of connA and connB are acceptable
			suite.Require().NotNil(connA)
			...
			suite.chainB.CreatePortCapability(connB.FirstOrNextTestChannel(icptesting.MockPort).PortID)
			portCap = suite.chainB.GetPortCapability(connB.FirstOrNextTestChannel(icptesting.MockPort).PortID)
		}, false},
		...
		{"connection version not negotiated", func() {
			...
			suite.chainB.App.ICPKeeper.ConnectionKeeper.SetConnection(
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			tc.malleate()
			channelA := connA.FirstOrNextTestChannel(icptesting.MockPort)
			channelB := connB.FirstOrNextTestChannel(icptesting.MockPort)
			...
			channelID, cap, err := suite.chainB.App.ICPKeeper.ChannelKeeper.ChanOpenTry(
				suite.chainB.GetContext(), types.ORDERED, []string{connB.ID},
				channelB.PortID, previousChannelID, portCap, counterparty, channelB.Version, connA.FirstOrNextTestChannel(icptesting.MockPort).Version,
			...
			if tc.expPass {
				...
				chanCap, ok := suite.chainB.App.ScopedICPKeeper.GetCapability(
					suite.chainB.GetContext(),
...
func (suite *KeeperTestSuite) TestChanOpenAck() {
	...
	testCases := []testCase{
		...
		{"success with empty stored counterparty channel ID", func() {
			...
			suite.chainA.App.ICPKeeper.ChannelKeeper.SetChannel(suite.chainA.GetContext(), channelA.PortID, channelA.ID, channel)
		...
		{"connection is not OPEN", func() {
			...
			channelA, _, err := suite.coordinator.ChanOpenInit(suite.chainA, suite.chainB, connA, connB, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
			suite.Require().NoError(err)
		...
		{"channel verification failed", func() {
			...
			_, _, connA, connB = suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
			channelB, channelA, err := suite.coordinator.ChanOpenInit(suite.chainB, suite.chainA, connB, connA, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
			suite.Require().NoError(err)
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			err := suite.chainA.App.ICPKeeper.ChannelKeeper.ChanOpenAck(
...
func (suite *KeeperTestSuite) TestChanOpenConfirm() {
	...
	testCases := []testCase{
		...
		{"connection not found", func() {
			...
			channel.ConnectionHops[0] = "doesnotexist"
			suite.chainB.App.ICPKeeper.ChannelKeeper.SetChannel(suite.chainB.GetContext(), channelB.PortID, channelB.ID, channel)
		}, false},
		{"connection is not OPEN", func() {
			clientA, clientB := suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			err := suite.chainB.App.ICPKeeper.ChannelKeeper.ChanOpenConfirm(
...
func (suite *KeeperTestSuite) TestChanCloseInit() {
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			err := suite.chainA.App.ICPKeeper.ChannelKeeper.ChanCloseInit(
...
func (suite *KeeperTestSuite) TestChanCloseConfirm() {
	var (
		connA      *icptesting.TestConnection
		connB      *icptesting.TestConnection
		channelA   icptesting.TestChannel
		channelB   icptesting.TestChannel
		channelCap *capabilitytypes.Capability
		heightDiff uint64
	)
	...
	testCases := []testCase{
		...
		{"connection is not OPEN", func() {
			...
			// create channel in init
			channelB, _, err := suite.coordinator.ChanOpenInit(suite.chainB, suite.chainA, connB, connA, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
			suite.Require().NoError(err)
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			err := suite.chainB.App.ICPKeeper.ChannelKeeper.ChanCloseConfirm(
