	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.chainA.App.ICPKeeper.ConnectionKeeper.SetConnection(suite.chainA.GetContext(), connA.ID, connectiontypes.ConnectionEnd{})
		...
		},
		{
			"success",
			func() {
				clientA, _, connA, connB := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
				// init channel
				channelA, _, err := suite.coordinator.ChanOpenInit(suite.chainA, suite.chainB, connA, connB, icptesting.MockPort, icptesting.MockPort, types.ORDERED)
				suite.Require().NoError(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(suite.chainA.GetContext(), channelA.PortID, channelA.ID, 1, expCommitment)
		...
		},
		{
			...
			func() {
				...
				for i := uint64(0); i < 9; i++ {
					...
					suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(suite.chainA.GetContext(), commitment.PortId, commitment.ChannelId, commitment.Sequence, commitment.Data)
...
func (suite *KeeperTestSuite) TestQueryPacketReceipt() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				_, _, _, _, channelA, _ := suite.coordinator.Setup(suite.chainA, suite.chainB, types.UNORDERED)
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketReceipt(suite.chainA.GetContext(), channelA.PortID, channelA.ID, 1)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				_, _, _, _, channelA, _ := suite.coordinator.Setup(suite.chainA, suite.chainB, types.UNORDERED)
				expAck = []byte("hash")
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketAcknowledgement(suite.chainA.GetContext(), channelA.PortID, channelA.ID, 1, expAck)
		...
		},
		{
			...
			func() {
				...
				for i := uint64(0); i < 9; i++ {
					...
					suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketAcknowledgement(suite.chainA.GetContext(), ack.PortId, ack.ChannelId, ack.Sequence, ack.Data)
		...
		},
		{
			...
			func() {
				...
				for seq := uint64(1); seq < 10; seq++ {
					...
					if seq%2 == 0 {
						suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketReceipt(suite.chainA.GetContext(), channelA.PortID, channelA.ID, seq)
					} else {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(suite.chainA.GetContext(), channelA.PortID, channelA.ID, 1, []byte("commitment"))
		...
		},
		{
			...
			func() {
				...
				for seq := uint64(1); seq < 10; seq++ {
					...
					if seq%2 == 0 {
						suite.chainA.App.ICPKeeper.ChannelKeeper.SetPacketCommitment(suite.chainA.GetContext(), channelA.PortID, channelA.ID, seq, []byte("commitement"))
						expSeq = append(expSeq, seq)
					}
				}
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				_, _, _, _, channelA, _ := suite.coordinator.Setup(suite.chainA, suite.chainB, types.UNORDERED)
				expSeq = 1
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetNextSequenceRecv(suite.chainA.GetContext(), channelA.PortID, channelA.ID, expSeq)
