...
func TestDecodeStore(t *testing.T) {
	...
	dec := simulation.NewDecodeStore(*app.ICPKeeper)
	...
	kvPairs := kv.Pairs{
		Pairs: []kv.Pair{
			...
			},
			{
				...
				Value: app.ICPKeeper.Codec().MustMarshalBinaryBare(&connection),
			},
			{
				Key:   host.ChannelKey(portID, channelID),
				Value: app.ICPKeeper.Codec().MustMarshalBinaryBare(&channel),
			},
			{
				Key:   []byte{0x99},
				Value: []byte{0x99},
