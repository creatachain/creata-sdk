...
// tests the ICP handler receiving a packet on ordered and unordered channels.
...
func (suite *KeeperTestSuite) TestHandleRecvPacket() {
	...
	testCases := []struct {
		...
		{"success: ORDERED", func() {
			...
			packet = channeltypes.NewPacket(icptesting.MockCommitment, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, timeoutHeight, 0)
		...
		{"success: UNORDERED out of order packet", func() {
			...
			for i := uint64(1); i < 10; i++ {
				packet = channeltypes.NewPacket(icptesting.MockCommitment, i, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, timeoutHeight, 0)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			_, err := keeper.Keeper.RecvPacket(*suite.chainB.App.ICPKeeper, sdk.WrapSDKContext(suite.chainB.GetContext()), msg)
			...
			if tc.expPass {
				...
				ack, found := suite.chainB.App.ICPKeeper.ChannelKeeper.GetPacketAcknowledgement(suite.chainB.GetContext(), packet.GetDestPort(), packet.GetDestChannel(), packet.GetSequence())
...
// tests the ICP handler acknowledgement of a packet on ordered and unordered
...
func (suite *KeeperTestSuite) TestHandleAcknowledgePacket() {
	...
	testCases := []struct {
		...
		{"ORDERED: packet already acknowledged (replay)", func() {
			...
			err = suite.coordinator.AcknowledgePacket(suite.chainA, suite.chainB, clientB, packet, icptesting.TestHash)
			suite.Require().NoError(err)
		}, false},
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			suite.SetupTest() // reset
			icptesting.TestHash = icptesting.MockAcknowledgement
			...
			msg := channeltypes.NewMsgAcknowledgement(packet, icpmock.MockAcknowledgement, proof, proofHeight, suite.chainA.SenderAccount.GetAddress())

			_, err := keeper.Keeper.Acknowledgement(*suite.chainA.App.ICPKeeper, sdk.WrapSDKContext(suite.chainA.GetContext()), msg)
			...
			if tc.expPass {
				...
				has := suite.chainA.App.ICPKeeper.ChannelKeeper.HasPacketCommitment(suite.chainA.GetContext(), packet.GetSourcePort(), packet.GetSourceChannel(), packet.GetSequence())
...
// tests the ICP handler timing out a packet on ordered and unordered channels.
...
func (suite *KeeperTestSuite) TestHandleTimeoutPacket() {
	...
	testCases := []struct {
		...
		{"success: ORDERED", func() {
			clientA, clientB, _, _, channelA, channelB := suite.coordinator.Setup(suite.chainA, suite.chainB, channeltypes.ORDERED)
			packet = channeltypes.NewPacket(icptesting.MockCommitment, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, clienttypes.GetSelfHeight(suite.chainB.GetContext()), uint64(suite.chainB.GetContext().BlockTime().UnixNano()))
		...
		{"success: UNORDERED timeout out of order packet", func() {
			...
			for i := uint64(1); i < maxSequence; i++ {
				packet = channeltypes.NewPacket(icptesting.MockCommitment, i, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, clienttypes.GetSelfHeight(suite.chainB.GetContext()), 0)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			_, err := keeper.Keeper.Timeout(*suite.chainA.App.ICPKeeper, sdk.WrapSDKContext(suite.chainA.GetContext()), msg)
...
// tests the ICP handler timing out a packet via channel closure on ordered
...
func (suite *KeeperTestSuite) TestHandleTimeoutOnClosePacket() {
	var (
		packet              channeltypes.Packet
		packetKey           []byte
		counterpartyChannel icptesting.TestChannel
	)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			_, err := keeper.Keeper.TimeoutOnClose(*suite.chainA.App.ICPKeeper, sdk.WrapSDKContext(suite.chainA.GetContext()), msg)
...
func (suite *KeeperTestSuite) TestUpgradeClient() {
	...
	cases := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				upgradedClient = icptmtypes.NewClientState("newChainId", icptmtypes.DefaultTrustLevel, icptesting.TrustingPeriod, icptesting.UnbondingPeriod+icptesting.TrustingPeriod, icptesting.MaxClockDrift, newClientHeight, commitmenttypes.GetSDKSpecs(), icptesting.UpgradePath, false, false)
				// Call ZeroCustomFields on upgraded clients to clear any client-chosen parameters in test-case upgradedClient
				upgradedClient = upgradedClient.ZeroCustomFields()

				upgradedConsState = &icptmtypes.ConsensusState{
				...
				cs, found := suite.chainA.App.ICPKeeper.ClientKeeper.GetClientState(suite.chainA.GetContext(), clientA)
				suite.Require().True(found)
	...
	for _, tc := range cases {
		...
		_, err := keeper.Keeper.UpgradeClient(*suite.chainA.App.ICPKeeper, sdk.WrapSDKContext(suite.chainA.GetContext()), msg)

		if tc.expPass {
			suite.Require().NoError(err, "upgrade handler failed on valid case: %s", tc.name)
			newClient, ok := suite.chainA.App.ICPKeeper.ClientKeeper.GetClientState(suite.chainA.GetContext(), clientA)
