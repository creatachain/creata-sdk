...
func (suite *TransferTestSuite) TestOnChanOpenInit() {
	var (
		...
		testChannel icptesting.TestChannel
	...
	testCases := []struct {
		...
		},
		{
			"invalid port ID", func() {
				testChannel = suite.chainA.NextTestChannel(connA, icptesting.MockPort)
			}, false,
		},
		{
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			testChannel = suite.chainA.NextTestChannel(connA, icptesting.TransferPort)
			...
			module, _, err := suite.chainA.App.ICPKeeper.PortKeeper.LookupModuleByPort(suite.chainA.GetContext(), icptesting.TransferPort)
			suite.Require().NoError(err)

			chanCap, err = suite.chainA.App.ScopedICPKeeper.NewCapability(suite.chainA.GetContext(), host.ChannelCapabilityPath(icptesting.TransferPort, testChannel.ID))
			suite.Require().NoError(err)

			cbs, ok := suite.chainA.App.ICPKeeper.Router.GetRoute(module)
