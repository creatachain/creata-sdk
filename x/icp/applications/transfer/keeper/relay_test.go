...
func (suite *KeeperTestSuite) TestSendTransfer() {
	var (
		amount             sdk.Coin
		channelA, channelB icptesting.TestChannel
		err                error
	)
	...
	testCases := []struct {
		...
		{"source channel not found",
			func() {
				...
				_, _, connA, connB := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
				channelA, channelB = suite.coordinator.CreateTransferChannels(suite.chainA, suite.chainB, connA, connB, channeltypes.UNORDERED)
				channelA.ID = icptesting.InvalidID
				amount = sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(100))
			}, true, false},
		{"next seq send not found",
			func() {
				...
				suite.chainA.App.ICPKeeper.ChannelKeeper.SetChannel(
					...
					channeltypes.NewChannel(channeltypes.OPEN, channeltypes.ORDERED, channeltypes.NewCounterparty(channelB.PortID, channelB.ID), []string{connA.ID}, icptesting.DefaultChannelVersion),
				)
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	testCases := []struct {
		...
		{"invalid receiver address", func() {
			receiver = "creata1scqhwpgsmr6vmztaa7suurfl52my6nd2kmrudl"
		}, true, false},
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			transferMsg := types.NewMsgTransfer(channelA.PortID, channelA.ID, sdk.NewCoin(trace.ICPDenom(), amount), suite.chainA.SenderAccount.GetAddress(), receiver, clienttypes.NewHeight(0, 110), 0)
...
func (suite *KeeperTestSuite) TestOnAcknowledgementPacket() {
	...
	testCases := []struct {
		...
		{"successful refund from with coin from external chain", failedAck,
			func() {
				escrow := types.GetEscrowAddress(channelA.PortID, channelA.ID)
				trace = types.ParseDenomTrace(types.GetPrefixedDenom(channelA.PortID, channelA.ID, sdk.DefaultBondDenom))
				coin := sdk.NewCoin(trace.ICPDenom(), amount)
	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			preCoin := suite.chainA.App.BankKeeper.GetBalance(suite.chainA.GetContext(), suite.chainA.SenderAccount.GetAddress(), trace.ICPDenom())

			err := suite.chainA.App.TransferKeeper.OnAcknowledgementPacket(suite.chainA.GetContext(), packet, data, tc.ack)
			if tc.expPass {
				suite.Require().NoError(err)
				postCoin := suite.chainA.App.BankKeeper.GetBalance(suite.chainA.GetContext(), suite.chainA.SenderAccount.GetAddress(), trace.ICPDenom())
...
// wrapper over it. The actual timeout does not matter since ICP core logic
