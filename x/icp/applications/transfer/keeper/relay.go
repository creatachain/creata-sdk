...
// to the receiving chain through ICP TAO logic. It is expected that the
// receiving chain will mint vouchers to the receiving address.
//
// 2. Sender chain is acting as the sink zone. The coins (vouchers) are burned
// on the sender chain and then transferred to the receiving chain though ICP
...
func (k Keeper) SendTransfer(
	...
	if strings.HasPrefix(token.Denom, "icp/") {
...
func (k Keeper) OnRecvPacket(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenPacketData) error {
	...
	if types.ReceiverChainIsSource(packet.GetSourcePort(), packet.GetSourceChannel(), data.Denom) {
		...
		defer func() {
			telemetry.SetGaugeWithLabels(
				[]string{"icp", types.ModuleName, "packet", "receive"},
				float32(data.Amount),
				[]metrics.Label{telemetry.NewLabel("denom", unprefixedDenom)},
			)

			telemetry.IncrCounterWithLabels(
				[]string{"icp", types.ModuleName, "receive"},
				1,
				append(
	...
	voucherDenom := denomTrace.ICPDenom()
...
func (k Keeper) refundPacketToken(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenPacketData) error {
	...
	token := sdk.NewCoin(trace.ICPDenom(), sdk.NewIntFromUint64(data.Amount))
...
// DenomPathFromHash returns the full denomination path prefix from an icp denom with a hash
// component.
func (k Keeper) DenomPathFromHash(ctx sdk.Context, denom string) (string, error) {
	// trim the denomination prefix, by default "icp/"
