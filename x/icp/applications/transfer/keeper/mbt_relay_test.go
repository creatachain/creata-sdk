...
/// Written by Andrey Kuprianov within the scope of ICP Audit performed by Informal Systems.
...
func BankOfChain(chain *icptesting.TestChain) Bank {
	bank := MakeBank()
	chain.App.BankKeeper.IterateAllBalances(chain.GetContext(), func(address sdk.AccAddress, coin sdk.Coin) (stop bool) {
		...
		if strings.HasPrefix(coin.Denom, "icp/") {
...
func (suite *KeeperTestSuite) SetChainBankBalances(chain *icptesting.TestChain, bank *Bank) error {
	for coin, amount := range bank.balances {
		...
		err = chain.App.BankKeeper.SetBalance(chain.GetContext(), address, sdk.NewCoin(trace.ICPDenom(), amount))
		if err != nil {
			return err
		}
	}
...
func (suite *KeeperTestSuite) CheckBankBalances(chain *icptesting.TestChain, bankBefore *Bank, expectedBankChange *Bank) error {
...
func (suite *KeeperTestSuite) TestModelBasedRelay() {
	...
	for _, file_info := range files {
		...
		suite.SetupTest()
		_, _, connAB, connBA := suite.coordinator.SetupClientConnections(suite.chainA, suite.chainB, exported.Augusteum)
		_, _, connBC, connCB := suite.coordinator.SetupClientConnections(suite.chainB, suite.chainC, exported.Augusteum)
