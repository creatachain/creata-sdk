...
func GetCmdQueryDenomTrace() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query icp-transfer denom-trace [hash]", version.AppName),
...
func GetCmdQueryDenomTraces() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query icp-transfer denom-traces", version.AppName),
		Args:    cobra.NoArgs,
...
// GetCmdParams returns the command handler for icp-transfer parameter querying.
func GetCmdParams() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "params",
		Short:   "Query the current icp-transfer parameters",
		Long:    "Query the current icp-transfer parameters",
		Args:    cobra.NoArgs,
		Example: fmt.Sprintf("%s query icp-transfer params", version.AppName),
		RunE: func(cmd *cobra.Command, _ []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
