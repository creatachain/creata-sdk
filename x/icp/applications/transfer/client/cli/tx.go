...
import (
	...
	channelutils "github.com/creatachain/creata-sdk/x/icp/core/04-channel/client/utils"
)
...
func NewTransferTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Transfer a fungible token through ICP",
		Long: strings.TrimSpace(`Transfer a fungible token through ICP. Timeouts can be specified
...
to the counterparty channel. Any timeout set to 0 is disabled.`),
		Example: fmt.Sprintf("%s tx icp-transfer transfer [src-port] [src-channel] [receiver] [amount]", version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !strings.HasPrefix(coin.Denom, "icp/") {
				...
				coin.Denom = denomTrace.ICPDenom()
			}
