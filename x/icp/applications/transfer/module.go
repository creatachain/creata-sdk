...
import (
	...
	"github.com/creatachain/creata-sdk/x/icp/applications/transfer/client/cli"
	"github.com/creatachain/creata-sdk/x/icp/applications/transfer/keeper"
...
var (
	...
	_ porttypes.ICPModule   = AppModule{}
	_ module.AppModuleBasic = AppModuleBasic{}
)

// AppModuleBasic is the ICP Transfer AppModuleBasic
...
// ValidateGenesis performs genesis state validation for the icp transfer module.
...
// RegisterGRPCGatewayRoutes registers the gRPC Gateway routes for the icp-transfer module.
...
// InitGenesis performs genesis initialization for the icp-transfer module. It returns
...
// ExportGenesis returns the exported genesis state as raw bytes for the icp-transfer
// module.
...
// RandomizedParams creates randomized icp-transfer param changes for the simulator.
...
func (am AppModule) OnChanOpenInit(
	...
	// Claim channel capability passed back by ICP module
...
func (am AppModule) OnChanOpenTry(
	...
	// Otherwise, module does not have channel capability and we must claim it from ICP
	if !am.keeper.AuthenticateCapability(ctx, chanCap, host.ChannelCapabilityPath(portID, channelID)) {
		// Only claim channel capability passed back by ICP module if we do not already own it
		if err := am.keeper.ClaimCapability(ctx, chanCap, host.ChannelCapabilityPath(portID, channelID)); err != nil {
			return err
		}
	}
...
func (am AppModule) OnRecvPacket(
	...
	// NOTE: acknowledgement will be written synchronously during ICP handler execution.
	return &sdk.Result{
		Events: ctx.EventManager().Events().ToMSMEvents(),
