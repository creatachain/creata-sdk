...
// ParseDenomTrace parses a string with the icp prefix (denom trace) and the base denomination
// into a DenomTrace type.
//
// Examples:
//
// 	- "portidone/channelidone/ucta" => DenomTrace{Path: "portidone/channelidone", BaseDenom: "ucta"}
// 	- "ucta" => DenomTrace{Path: "", BaseDenom: "ucta"}
...
// ICPDenom a coin denomination for an ICS20 fungible token in the format
// 'icp/{hash(tracePath + baseDenom)}'. If the trace is empty, it will return the base denomination.
func (dt DenomTrace) ICPDenom() string {
	if dt.Path != "" {
...
// ValidatePrefixedDenom checks that the denomination for an ICP fungible token packet denom is correctly prefixed.
...
// ValidateICPDenom validates that the given denomination is either:
//
//  - A valid base denomination (eg: 'ucta')
//  - A valid fungible token representation (i.e 'icp/{hash}') per ADR 001 https://github.com/creatachain/creata-sdk/blob/master/docs/architecture/adr-001-coin-source-tracing.md
func ValidateICPDenom(denom string) error {
	...
	case strings.TrimSpace(denom) == "",
		...
		return sdkerrors.Wrapf(ErrInvalidDenomForTransfer, "denomination should be prefixed with the format 'icp/{hash(trace + \"/\" + %s)}'", denom)
