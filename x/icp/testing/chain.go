...
import (
	...
	tmprotoversion "github.com/creatachain/augusteum/proto/augusteum/version"
	tmtypes "github.com/creatachain/augusteum/types"
	tmversion "github.com/creatachain/augusteum/version"
	...
	"github.com/creatachain/creata-sdk/x/icp/testing/mock"
...
const (
	...
	DefaultChannelVersion = icptransfertypes.Version
	InvalidID             = "IDisInvalid"

	ConnectionIDPrefix = "conn"
	ChannelIDPrefix    = "chan"

	TransferPort = icptransfertypes.ModuleName
...
var (
	...
	DefaultTrustLevel icptmtypes.Fraction = icptmtypes.DefaultTrustLevel
...
// TestChain is a testing struct that wraps a creataapp with the last TM Header, the current MSM
...
type TestChain struct {
	...
	ChainID       string
	LastHeader    *icptmtypes.Header // header for last block height committed
...
func NewTestChain(t *testing.T, chainID string) *TestChain {
	...
	app := creataapp.SetupWithGenesisValSet(t, valSet, []authtypes.GenesisAccount{acc}, balance)
	...
	txConfig := creataapp.MakeTestEncodingConfig().TxConfig
	...
	cap := chain.App.ICPKeeper.PortKeeper.BindPort(chain.GetContext(), MockPort)
	err = chain.App.ScopedICPMockKeeper.ClaimCapability(chain.GetContext(), cap, host.PortPath(MockPort))
	require.NoError(t, err)
...
// QueryProof performs an msm query with the given key and returns the proto encoded merkle proof
// for the query and the height at which the proof will succeed on a augusteum verifier.
func (chain *TestChain) QueryProof(key []byte) ([]byte, clienttypes.Height) {
	res := chain.App.Query(msm.RequestQuery{
	...
	// was created in the IAVL tree. Augusteum and subsequently the clients that rely on it
	// have heights 1 above the IAVL tree. Thus we return proof height + 1
	return proof, clienttypes.NewHeight(revision, uint64(res.Height)+1)
}

// QueryUpgradeProof performs an msm query with the given key and returns the proto encoded merkle proof
// for the query and the height at which the proof will succeed on a augusteum verifier.
...
// QueryClientStateProof performs and msm query for a client state
...
func (chain *TestChain) QueryClientStateProof(clientID string) (exported.ClientState, []byte) {
	...
	clientState, found := chain.App.ICPKeeper.ClientKeeper.GetClientState(chain.GetContext(), clientID)
...
// QueryConsensusStateProof performs an msm query for a consensus state
...
func (chain *TestChain) NextBlock() {
	...
	chain.App.BeginBlock(msm.RequestBeginBlock{Header: chain.CurrentHeader})
...
func (chain *TestChain) SendMsgs(msgs ...sdk.Msg) (*sdk.Result, error) {
	_, r, err := creataapp.SignCheckDeliver(
		chain.t,
		chain.TxConfig,
		chain.App.BaseApp,
...
func (chain *TestChain) GetConsensusState(clientID string, height exported.Height) (exported.ConsensusState, bool) {
	return chain.App.ICPKeeper.ClientKeeper.GetClientConsensusState(chain.GetContext(), clientID, height)
}
...
// GetConnection retrieves an ICP Connection for the provided TestConnection. The
// connection is expected to exist otherwise testing will fail.
func (chain *TestChain) GetConnection(testConnection *TestConnection) connectiontypes.ConnectionEnd {
	connection, found := chain.App.ICPKeeper.ConnectionKeeper.GetConnection(chain.GetContext(), testConnection.ID)
	require.True(chain.t, found)

	return connection
}

// GetChannel retrieves an ICP Channel for the provided TestChannel. The channel
// is expected to exist otherwise testing will fail.
func (chain *TestChain) GetChannel(testChannel TestChannel) channeltypes.Channel {
	channel, found := chain.App.ICPKeeper.ChannelKeeper.GetChannel(chain.GetContext(), testChannel.PortID, testChannel.ID)
	require.True(chain.t, found)
...
func (chain *TestChain) GetAcknowledgement(packet exported.PacketI) []byte {
	ack, found := chain.App.ICPKeeper.ChannelKeeper.GetPacketAcknowledgement(chain.GetContext(), packet.GetDestPort(), packet.GetDestChannel(), packet.GetSequence())
	require.True(chain.t, found)
...
func (chain *TestChain) GetPrefix() commitmenttypes.MerklePrefix {
	return commitmenttypes.NewMerklePrefix(chain.App.ICPKeeper.ConnectionKeeper.GetCommitmentPrefix().Bytes())
}
...
func (chain *TestChain) NextTestChannel(conn *TestConnection, portID string) TestChannel {
	nextChanSeq := chain.App.ICPKeeper.ChannelKeeper.GetNextChannelSequence(chain.GetContext())
...
// ConstructMsgCreateClient constructs a message to create a new client state (augusteum or solomachine).
...
// CreateTMClient will construct and execute a 07-augusteum MsgCreateClient. A counterparty
...
func (chain *TestChain) CreateTMClient(counterparty *TestChain, clientID string) error {
	...
	msg := chain.ConstructMsgCreateClient(counterparty, clientID, exported.Augusteum)
	return chain.sendMsgs(msg)
}

// UpdateTMClient will construct and execute a 07-augusteum MsgUpdateClient. The counterparty
// client will be updated on the (target) chain. UpdateTMClient mocks the relayer flow
// necessary for updating a Augusteum client.
...
// ConstructUpdateTMClientHeader will construct a valid 07-augusteum Header to update the
...
func (chain *TestChain) ConstructUpdateTMClientHeader(counterparty *TestChain, clientID string) (*icptmtypes.Header, error) {
	...
	if trustedHeight == counterparty.LastHeader.GetHeight() {
		...
		if !ok {
			return nil, sdkerrors.Wrapf(icptmtypes.ErrInvalidHeaderHeight, "could not retrieve trusted validators at trustedHeight: %d", trustedHeight)
		}
	}
...
func (chain *TestChain) CreatePortCapability(portID string) {
	...
	_, ok := chain.App.ScopedICPKeeper.GetCapability(chain.GetContext(), host.PortPath(portID))
	if !ok {
		// create capability using the ICP capability keeper
		cap, err := chain.App.ScopedICPKeeper.NewCapability(chain.GetContext(), host.PortPath(portID))
		require.NoError(chain.t, err)

		switch portID {
		case MockPort:
			// claim capability using the mock capability keeper
			err = chain.App.ScopedICPMockKeeper.ClaimCapability(chain.GetContext(), cap, host.PortPath(portID))
			require.NoError(chain.t, err)
		case TransferPort:
			...
			require.NoError(chain.t, err)
		default:
			panic(fmt.Sprintf("unsupported icp testing package port ID %s", portID))
		}
	}
...
func (chain *TestChain) GetPortCapability(portID string) *capabilitytypes.Capability {
	cap, ok := chain.App.ScopedICPKeeper.GetCapability(chain.GetContext(), host.PortPath(portID))
...
func (chain *TestChain) CreateChannelCapability(portID, channelID string) {
	...
	// check if the portId is already binded, if not bind it
	_, ok := chain.App.ScopedICPKeeper.GetCapability(chain.GetContext(), capName)
	if !ok {
		cap, err := chain.App.ScopedICPKeeper.NewCapability(chain.GetContext(), capName)
		require.NoError(chain.t, err)
...
func (chain *TestChain) GetChannelCapability(portID, channelID string) *capabilitytypes.Capability {
	cap, ok := chain.App.ScopedICPKeeper.GetCapability(chain.GetContext(), host.ChannelCapabilityPath(portID, channelID))
	require.True(chain.t, ok)
...
// GetPacketData returns a icp-transfer marshalled packet to be used for
// callback testing.
func (chain *TestChain) GetPacketData(counterparty *TestChain) []byte {
	packet := icptransfertypes.FungibleTokenPacketData{
...
func (chain *TestChain) SendPacket(
	...
	err := chain.App.ICPKeeper.ChannelKeeper.SendPacket(chain.GetContext(), channelCap, packet)
	if err != nil {
		return err
	}
...
func (chain *TestChain) WriteAcknowledgement(
	...
	err := chain.App.ICPKeeper.ChannelKeeper.WriteAcknowledgement(chain.GetContext(), channelCap, packet, TestHash)
	if err != nil {
		return err
	}
