...
// NewCreateClientCmd defines the command to create a new ICP Client as defined

func NewCreateClientCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create [path/to/consensus_state.json] [trusting_period] [unbonding_period] [max_clock_drift]",
		Short: "create new augusteum client",
		Long: `Create a new augusteum ICP client.
...
func NewUpdateClientCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "update existing augusteum client with a augusteum header",
		Example: fmt.Sprintf(
			"$ %s tx icp %s update [client-id] [path/to/header.json] --from node0 --home ../node0/<app>cli --chain-id $CID",
			version.AppName, types.SubModuleName,
		),
...
func NewSubmitMisbehaviourCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(
			"$ %s tx icp %s misbehaviour [path/to/misbehaviour.json] --from node0 --home ../node0/<app>cli --chain-id $CID",
			version.AppName, types.SubModuleName,
		),
