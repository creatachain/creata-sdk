...
func (suite *AugusteumTestSuite) TestCheckMisbehaviourAndUpdateState() {
	...
	for i, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case: %s", tc.name), func() {
			...
			if tc.consensusState1 != nil {
				suite.chainA.App.ICPKeeper.ClientKeeper.SetClientConsensusState(ctx, clientID, tc.height1, tc.consensusState1)
			}
			if tc.consensusState2 != nil {
				suite.chainA.App.ICPKeeper.ClientKeeper.SetClientConsensusState(ctx, clientID, tc.height2, tc.consensusState2)
			}

			clientState, err := tc.clientState.CheckMisbehaviourAndUpdateState(
				ctx,
				suite.cdc,
				suite.chainA.App.ICPKeeper.ClientKeeper.ClientStore(ctx, clientID), // pass in clientID prefixed clientStore
				tc.misbehaviour,
			)
