...
// in client state that must be the same across all valid Augusteum clients for the new chain.
// VerifyUpgrade will return an error if:
// - the upgradedClient is not a Augusteum ClientState
...
// - any Augusteum chain specified parameter in upgraded client such as ChainID, UnbondingPeriod,
...
func (cs ClientState) VerifyUpgradeAndUpdateState(
	...
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidClientType, "upgraded client must be Augusteum client. expected: %T got: %T",
			&ClientState{}, upgradedClient)
	}
	tmUpgradeConsState, ok := upgradedConsState.(*ConsensusState)
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidConsensus, "upgraded consensus state must be Augusteum consensus state. expected %T, got: %T",
