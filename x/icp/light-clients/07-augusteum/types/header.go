...
// ClientType defines that the Header is a Augusteum consensus algorithm
func (h Header) ClientType() string {
	return exported.Augusteum
}

// GetHeight returns the current height. It returns 0 if the augusteum
// header is nil.
...
func (h Header) ValidateBasic() error {
	if h.SignedHeader == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "augusteum signed header cannot be nil")
	}
	if h.Header == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "augusteum header cannot be nil")
	}
	tmSignedHeader, err := tmtypes.SignedHeaderFromProto(h.SignedHeader)
	if err != nil {
		return sdkerrors.Wrap(err, "header is not a augusteum header")
	}
	...
	if err != nil {
		return sdkerrors.Wrap(err, "validator set is not augusteum validator set")
	}
