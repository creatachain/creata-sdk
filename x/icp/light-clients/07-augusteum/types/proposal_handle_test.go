...
// sanity checks
func (suite *AugusteumTestSuite) TestCheckProposedHeaderAndUpdateStateBasic() {
	...
	clientStore := suite.chainA.App.ICPKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), clientA)
...
func (suite *AugusteumTestSuite) TestCheckProposedHeaderAndUpdateState() {
	testCases := []struct {
		name                         string
...
func (suite *AugusteumTestSuite) TestCheckProposedHeader() {
	var (
		header      *types.Header
	...
	testCases := []struct {
		...
		},
		{
			"header time is less than or equal to consensus state timestamp", func() {
				...
				suite.chainA.App.ICPKeeper.ClientKeeper.SetClientConsensusState(suite.chainA.GetContext(), clientA, clientState.GetLatestHeight(), consensusState)
		...
		},
		{
			"signed header failed validate basic - wrong chain ID", func() {
				clientState.ChainId = icptesting.InvalidID
			}, false,
		},
		{
