...
func (suite *AugusteumTestSuite) TestValidate() {
	testCases := []struct {
		name        string
		...
		},
		{
			...
			clientState: types.NewClientState(chainID, types.DefaultTrustLevel, ubdPeriod, ubdPeriod, maxClockDrift, height, []*ics23.ProofSpec{ics23.AugusteumSpec, nil}, upgradePath, false, false),
			expPass:     false,
		},
	}
...
func (suite *AugusteumTestSuite) TestInitialize() {
	...
	testCases := []struct {
		...
		},
		{
			...
			consensusState: icptesting.NewSolomachine(suite.T(), suite.chainA.Codec, "solomachine", "", 2).ConsensusState(),
			expPass:        false,
		},
	}
...
func (suite *AugusteumTestSuite) TestVerifyClientConsensusState() {
	testCases := []struct {
		name           string
		clientState    *types.ClientState
		consensusState *types.ConsensusState
		...
		// 	},
		// 	prefix:  commitmenttypes.NewMerklePrefix([]byte("icp")),
		// 	expPass: true,
		// },
		{
...
func (suite *AugusteumTestSuite) TestVerifyConnectionState() {
	var (
		clientState *types.ClientState
		proof       []byte
...
// client on chainA
func (suite *AugusteumTestSuite) TestVerifyChannelState() {
	var (
		clientState *types.ClientState
		proof       []byte
		proofHeight exported.Height
...
func (suite *AugusteumTestSuite) TestVerifyPacketCommitment() {
	var (
		clientState *types.ClientState
		proof       []byte
		delayPeriod uint64
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			packet := channeltypes.NewPacket(icptesting.TestHash, 1, channelB.PortID, channelB.ID, channelA.PortID, channelA.ID, clienttypes.NewHeight(0, 100), 0)
			...
			commitment := channeltypes.CommitPacket(suite.chainA.App.ICPKeeper.Codec(), packet)
...
// is simulated.
func (suite *AugusteumTestSuite) TestVerifyPacketAcknowledgement() {
	var (
		clientState *types.ClientState
		proof       []byte
		delayPeriod uint64
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			packet := channeltypes.NewPacket(icptesting.TestHash, 1, channelA.PortID, channelA.ID, channelB.PortID, channelB.ID, clienttypes.NewHeight(0, 100), 0)
			...
			err = clientState.VerifyPacketAcknowledgement(
				store, suite.chainA.Codec, proofHeight, currentTime, delayPeriod, &prefix, proof,
				packet.GetDestPort(), packet.GetDestChannel(), packet.GetSequence(), icpmock.MockAcknowledgement,
			)
...
// no receive.
func (suite *AugusteumTestSuite) TestVerifyPacketReceiptAbsence() {
	var (
		clientState *types.ClientState
		proof       []byte
		delayPeriod uint64
...
// simulated.
func (suite *AugusteumTestSuite) TestVerifyNextSeqRecv() {
	var (
		clientState *types.ClientState
		proof       []byte
		delayPeriod uint64
