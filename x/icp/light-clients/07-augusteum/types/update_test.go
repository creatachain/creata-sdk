...
import (
	...
	types "github.com/creatachain/creata-sdk/x/icp/light-clients/07-augusteum/types"
...
func (suite *AugusteumTestSuite) TestCheckHeaderAndUpdateState() {
	var (
	...
	for i, tc := range testCases {
		...
		suite.chainA.App.ICPKeeper.ClientKeeper.SetClientConsensusState(ctx, clientID, consStateHeight, consensusState)
		...
		newClientState, consensusState, err := clientState.CheckHeaderAndUpdateState(
			ctx,
			suite.cdc,
			suite.chainA.App.ICPKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), clientID), // pass in clientID prefixed clientStore
			newHeader,
		)
