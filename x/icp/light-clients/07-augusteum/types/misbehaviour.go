...
// ClientType is Augusteum light client
...
func validCommit(chainID string, blockID tmtypes.BlockID, commit *tmproto.Commit, valSet *tmproto.ValidatorSet) (err error) {
	...
	if err != nil {
		return sdkerrors.Wrap(err, "commit is not augusteum commit type")
	}
	tmValset, err := tmtypes.ValidatorSetFromProto(valSet)
	if err != nil {
		return sdkerrors.Wrap(err, "validator set is not augusteum validator set type")
	}
