...
// - the client or header provided are not parseable to augusteum types
...
// Augusteum client validity checking uses the bisection algorithm described
// in the [Augusteum spec](https://github.com/augusteum/spec/blob/master/spec/consensus/light-client.md).
...
func checkTrustedHeader(header *Header, consState *ConsensusState) error {
	...
	if err != nil {
		return sdkerrors.Wrap(err, "trusted validator set in not augusteum validator set type")
	}
...
// checkValidity checks if the Augusteum header is valid.
...
func checkValidity(
	...
	if err != nil {
		return sdkerrors.Wrap(err, "signed header in not augusteum signed header type")
	}

	tmValidatorSet, err := tmtypes.ValidatorSetFromProto(header.ValidatorSet)
	if err != nil {
		return sdkerrors.Wrap(err, "validator set in not augusteum validator set type")
	}
	...
	err = light.Verify(
		...
		clientState.TrustingPeriod, currentTimestamp, clientState.MaxClockDrift, clientState.TrustLevel.ToAugusteum(),
	)
	if err != nil {
...
func update(ctx sdk.Context, clientStore sdk.KVStore, clientState *ClientState, header *Header) (*ClientState, *ConsensusState) {
	...
	// set context time as processed time as this is state internal to augusteum client logic.
