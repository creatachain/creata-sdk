...
func (cs ClientState) Validate() error {
	...
	if err := light.ValidateTrustLevel(cs.TrustLevel.ToAugusteum()); err != nil {
		return err
	}
	...
	if cs.LatestHeight.RevisionHeight == 0 {
		return sdkerrors.Wrapf(ErrInvalidHeaderHeight, "augusteum revision height cannot be zero")
	}
...
// Initialize will check that initial consensus state is a Augusteum consensus state
...
// Augusteum client stored on the target machine.
