...
func (suite *AugusteumTestSuite) TestMisbehaviour() {
	...
	suite.Require().Equal(exported.Augusteum, misbehaviour.ClientType())
	suite.Require().Equal(clientID, misbehaviour.GetClientID())
	suite.Require().Equal(height, misbehaviour.GetHeight())
}

func (suite *AugusteumTestSuite) TestMisbehaviourValidateBasic() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func(misbehaviour *types.Misbehaviour) error {
				tmBlockID := icptesting.MakeBlockID(tmhash.Sum([]byte("other_hash")), 3, tmhash.Sum([]byte("other_partset")))
