...
func (suite *SoloMachineTestSuite) TestInitialize() {
	...
	for _, solomachine := range []*icptesting.Solomachine{suite.solomachine, suite.solomachineMulti} {
		...
		testCases := []struct {
			...
			{
				"invalid consensus state: Augusteum consensus state",
				&icptmtypes.ConsensusState{},
				false,
			},
			{
		...
		for _, tc := range testCases {
			err := solomachine.ClientState().Initialize(
				...
				suite.chainA.App.ICPKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), "solomachine"),
				tc.consState,
			)
...
func (suite *SoloMachineTestSuite) TestVerifyClientState() {
	// create client for augusteum so we can use client state for verification
...
func (suite *SoloMachineTestSuite) TestVerifyClientConsensusState() {
	// create client for augusteum so we can use consensus state for verification
	clientA, _ := suite.coordinator.SetupClients(suite.chainA, suite.chainB, exported.Augusteum)
	clientState := suite.chainA.GetClientState(clientA)
