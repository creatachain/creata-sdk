...
func NewCreateClientCmd() *cobra.Command {
	...
	- ConsensusState json example: {"public_key":{"@type":"/creata.crypto.secp256k1.PubKey","key":"A/3SXL2ONYaOkxpdR5P8tHTlSlPv1AwQwSFxKRee5JQW"},"diversifier":"diversifier","timestamp":"10"}`,
		Example: fmt.Sprintf("%s tx icp %s create [sequence] [path/to/consensus_state] --from node0 --home ../node0/<app>cli --chain-id $CID", version.AppName, types.SubModuleName),
...
func NewUpdateClientCmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf("%s tx icp %s update [client-id] [path/to/header.json] --from node0 --home ../node0/<app>cli --chain-id $CID", version.AppName, types.SubModuleName),
		Args:    cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func NewSubmitMisbehaviourCmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf("%s tx icp %s misbehaviour [path/to/misbehaviour.json] --from node0 --home ../node0/<app>cli --chain-id $CID", version.AppName, types.SubModuleName),
