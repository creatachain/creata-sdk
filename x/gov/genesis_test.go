...
func TestImportExportQueues(t *testing.T) {
	...
	addrs := creataapp.AddTestAddrs(app, ctx, 2, valTokens)
	...
	db := dbm.NewMemDB()
	app2 := creataapp.NewCreataApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, creataapp.DefaultNodeHome, 0, creataapp.MakeTestEncodingConfig(), creataapp.EmptyAppOptions{})
	...
	app2.Commit()
	app2.BeginBlock(msm.RequestBeginBlock{Header: tmproto.Header{Height: app2.LastBlockHeight() + 1}})

	header = tmproto.Header{Height: app2.LastBlockHeight() + 1}
	app2.BeginBlock(msm.RequestBeginBlock{Header: header})
