...
func (s *IntegrationTestSuite) TestCmdParams() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			`{"voting_params":{"voting_period":"172800000000000"},"tally_params":{"quorum":"0.334000000000000000","threshold":"0.500000000000000000","veto_threshold":"0.334000000000000000"},"deposit_params":{"min_deposit":[{"denom":"ucta","amount":"10000000"}],"max_deposit_period":"172800000000000"}}`,
		},
		{
			"text output",
			[]string{},
...
func (s *IntegrationTestSuite) TestCmdParam() {
	...
	testCases := []struct {
		...
		},
		{
			...
			`{"min_deposit":[{"denom":"ucta","amount":"10000000"}],"max_deposit_period":"172800000000000"}`,
		},
	}
...
func (s *IntegrationTestSuite) TestNewCmdDeposit() {
	...
	testCases := []struct {
	...
	}{
		{
			"without proposal id",
			[]string{
				sdk.NewCoin(s.cfg.BondDenom, sdk.NewInt(10)).String(), //10fcta
				fmt.Sprintf("--%s=%s", flags.FlagFrom, val.Address.String()),
				fmt.Sprintf("--%s=true", flags.FlagSkipConfirmation),
				fmt.Sprintf("--%s=%s", flags.FlagBroadcastMode, flags.BroadcastBlock),
				fmt.Sprintf("--%s=%s", flags.FlagFees, sdk.NewCoins(sdk.NewCoin(s.cfg.BondDenom, sdk.NewInt(10))).String()),
