...
func (k Keeper) BlockValidatorUpdates(ctx sdk.Context) []msm.ValidatorUpdate {
...
// are returned to Augusteum.
func (k Keeper) ApplyAndReturnValidatorSetUpdates(ctx sdk.Context) (updates []msm.ValidatorUpdate, err error) {
	...
	for count := 0; iterator.Valid() && count < int(maxValidators); iterator.Next() {
		...
		if !found || !bytes.Equal(oldPowerBytes, newPowerBytes) {
			updates = append(updates, validator.MSMValidatorUpdate())
	...
	for _, valAddrBytes := range noLongerBonded {
		...
		updates = append(updates, validator.MSMValidatorUpdateZero())
	}
