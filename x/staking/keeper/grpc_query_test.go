...
func (suite *KeeperTestSuite) TestGRPCQueryValidatorDelegations() {
	...
	valAddrs := creataapp.ConvertAddrsToValAddrs(addrs)
	addrVal2 := valAddrs[4]
	valAddr, err := sdk.ValAddressFromBech32(addrVal1)
	suite.NoError(err)
	delegation, found := app.StakingKeeper.GetDelegation(ctx, addrAcc, valAddr)
...
func createValidators(t *testing.T, ctx sdk.Context, app *creataapp.CreataApp, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	addrs := creataapp.AddTestAddrsIncremental(app, ctx, 5, sdk.TokensFromConsensusPower(300))
	valAddrs := creataapp.ConvertAddrsToValAddrs(addrs)
