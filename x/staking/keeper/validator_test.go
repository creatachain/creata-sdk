...
func bootstrapValidatorTest(t testing.TB, power int64, numAddrs int) (*creataapp.CreataApp, sdk.Context, []sdk.AccAddress, []sdk.ValAddress) {
...
func initValidators(t testing.TB, power int64, numAddrs int, powers []int64) (*creataapp.CreataApp, sdk.Context, []sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	app, ctx, addrs, valAddrs := bootstrapValidatorTest(t, power, numAddrs)
	pks := creataapp.CreateTestPubKeys(numAddrs)
...
func TestSetValidator(t *testing.T) {
	...
	require.Equal(t, validator.MSMValidatorUpdate(), updates[0])
...
func TestApplyAndReturnValidatorSetUpdatesAllNone(t *testing.T) {
	...
	assert.Equal(t, validators[0].MSMValidatorUpdate(), updates[1])
	assert.Equal(t, validators[1].MSMValidatorUpdate(), updates[0])
}
...
func TestApplyAndReturnValidatorSetUpdatesSingleValueChange(t *testing.T) {
	...
	//  augusteumUpdate set: {} -> {c1'}
	validators[0].Status = types.Bonded
	validators[0].Tokens = sdk.TokensFromConsensusPower(600)
	validators[0] = keeper.TestingUpdateValidator(app.StakingKeeper, ctx, validators[0], false)

	updates := applyValidatorSetUpdates(t, ctx, app.StakingKeeper, 1)
	require.Equal(t, validators[0].MSMValidatorUpdate(), updates[0])
}
...
func TestApplyAndReturnValidatorSetUpdatesMultipleValueChange(t *testing.T) {
	...
	//  augusteumUpdate set: {c1, c3} -> {c1', c3'}
	...
	updates := applyValidatorSetUpdates(t, ctx, app.StakingKeeper, 2)
	require.Equal(t, validators[0].MSMValidatorUpdate(), updates[0])
	require.Equal(t, validators[1].MSMValidatorUpdate(), updates[1])
}
...
func TestApplyAndReturnValidatorSetUpdatesInserted(t *testing.T) {
	...
	require.Equal(t, validators[2].MSMValidatorUpdate(), updates[0])
	...
	require.Equal(t, validators[3].MSMValidatorUpdate(), updates[0])
	...
	require.Equal(t, validators[4].MSMValidatorUpdate(), updates[0])
}
...
func TestApplyAndReturnValidatorSetUpdatesWithCliffValidator(t *testing.T) {
	...
	updates := applyValidatorSetUpdates(t, ctx, app.StakingKeeper, 2)
	validators[2], _ = app.StakingKeeper.GetValidator(ctx, validators[2].GetOperator())
	require.Equal(t, validators[0].MSMValidatorUpdateZero(), updates[1])
	require.Equal(t, validators[2].MSMValidatorUpdate(), updates[0])
}
...
func TestApplyAndReturnValidatorSetUpdatesPowerDecrease(t *testing.T) {
	...
	// Augusteum updates should reflect power change
	updates := applyValidatorSetUpdates(t, ctx, app.StakingKeeper, 2)
	require.Equal(t, validators[0].MSMValidatorUpdate(), updates[0])
	require.Equal(t, validators[1].MSMValidatorUpdate(), updates[1])
}
...
func TestApplyAndReturnValidatorSetUpdatesNewValidator(t *testing.T) {
	...
	// verify initial Augusteum updates are correct
	...
	validators[1], _ = app.StakingKeeper.GetValidator(ctx, validators[1].GetOperator())
	require.Equal(t, validator.MSMValidatorUpdate(), updates[0])
	require.Equal(t, validators[0].MSMValidatorUpdate(), updates[1])
	require.Equal(t, validators[1].MSMValidatorUpdate(), updates[2])
}
...
func TestApplyAndReturnValidatorSetUpdatesBondTransition(t *testing.T) {
	...
	// verify initial Augusteum updates are correct
	updates = applyValidatorSetUpdates(t, ctx, app.StakingKeeper, 1)
	require.Equal(t, validators[1].MSMValidatorUpdate(), updates[0])
...
func applyValidatorSetUpdates(t *testing.T, ctx sdk.Context, k keeper.Keeper, expectedUpdatesLen int) []msm.ValidatorUpdate {
