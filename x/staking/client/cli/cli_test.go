...
import (
	...
	"github.com/creatachain/augusteum/proto/augusteum/crypto"
	"github.com/creatachain/augusteum/rpc/client/http"
	...
	banktestutil "github.com/creatachain/creata-sdk/x/bank/client/testutil"
...
func (s *IntegrationTestSuite) TestNewCreateValidatorCmd() {
	...
	testCases := []struct {
		...
		},
		{
			...
			[]string{
				fmt.Sprintf("--%s=100fcta", cli.FlagAmount),
				fmt.Sprintf("--%s=AFAF00C4", cli.FlagIdentity),
				fmt.Sprintf("--%s=https://newvalidator.io", cli.FlagWebsite),
				fmt.Sprintf("--%s=contact@newvalidator.io", cli.FlagSecurityContact),
				fmt.Sprintf("--%s='Hey, I am a new validator. Please delegate!'", cli.FlagDetails),
...
func (s *IntegrationTestSuite) TestGetCmdQueryValidator() {
	...
	testCases := []struct {
		...
		},
		{
			...
			[]string{"creatavaloper15jkng8hytwt22lllv6mw4k89qkqehtahd84ptu", fmt.Sprintf("--%s=json", tmcli.OutputFlag)},
			true,
		},
		{
			"happy case",
...
unbonding_time: 1814400s`,
		},
		{
			"with json output",
			[]string{fmt.Sprintf("--%s=json", tmcli.OutputFlag)},
			`{"unbonding_time":"1814400s","max_validators":100,"max_entries":7,"historical_entries":10000,"bond_denom":"ucta"}`,
		},
	}
	for _, tc := range testCases {
		tc := tc
...
func (s *IntegrationTestSuite) TestNewCmdRedelegate() {
	...
	testCases := []struct {
		...
		},
		{
			...
			[]string{
				`creatavaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj`, // src-validator-addr
				val2.ValAddress.String(),                               // dst-validator-addr
