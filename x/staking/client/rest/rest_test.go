...
func (s *IntegrationTestSuite) TestLegacyGetValidators() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			true, "creata sdk v0.40 introduces a breaking change on this endpoint: instead of" +
				" querying using `?status=bonded`, please use `status=BOND_STATUS_BONDED`. For more" +
				" info, please see our REST endpoint migration guide at https://docs.creata.network/master/migrations/rest.html",
		},
		{
