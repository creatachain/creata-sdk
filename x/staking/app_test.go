...
func checkValidator(t *testing.T, app *creataapp.CreataApp, addr sdk.ValAddress, expFound bool) types.Validator {
...
func checkDelegation(
	t *testing.T, app *creataapp.CreataApp, delegatorAddr sdk.AccAddress,
...
func TestStakingMsgs(t *testing.T) {
	...
	creataapp.CheckBalance(t, app, addr2, sdk.Coins{genCoin})
	...
	header = tmproto.Header{Height: app.LastBlockHeight() + 1}
	_, _, err = creataapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{editValidatorMsg}, "", []uint64{0}, []uint64{1}, true, true, priv1)
	require.NoError(t, err)
	...
	header = tmproto.Header{Height: app.LastBlockHeight() + 1}
	_, _, err = creataapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{delegateMsg}, "", []uint64{1}, []uint64{0}, true, true, priv2)
	require.NoError(t, err)

	creataapp.CheckBalance(t, app, addr2, sdk.Coins{genCoin.Sub(bondCoin)})
	...
	header = tmproto.Header{Height: app.LastBlockHeight() + 1}
	_, _, err = creataapp.SignCheckDeliver(t, txGen, app.BaseApp, header, []sdk.Msg{beginUnbondingMsg}, "", []uint64{1}, []uint64{1}, true, true, priv2)
	require.NoError(t, err)
