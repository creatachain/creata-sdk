...
func TestUpdateDescription(t *testing.T) {
	d1 := types.Description{
		Website: "https://validator.creata",
...
func TestMSMValidatorUpdate(t *testing.T) {
	validator := newValidator(t, valAddr1, pk1)
	msmVal := validator.MSMValidatorUpdate()
	pk, err := validator.TmConsPublicKey()
	require.NoError(t, err)
	require.Equal(t, pk, msmVal.PubKey)
	require.Equal(t, validator.BondedTokens().Int64(), msmVal.Power)
}

func TestMSMValidatorUpdateZero(t *testing.T) {
	validator := newValidator(t, valAddr1, pk1)
	msmVal := validator.MSMValidatorUpdateZero()
	pk, err := validator.TmConsPublicKey()
	require.NoError(t, err)
	require.Equal(t, pk, msmVal.PubKey)
	require.Equal(t, int64(0), msmVal.Power)
}
...
func TestValidatorMarshalUnmarshalJSON(t *testing.T) {
	...
	require.Contains(t, string(js), "\"consensus_pubkey\":{\"type\":\"augusteum/PubKeyEd25519\"")
	got := &types.Validator{}
...
// Check SortAugusteum sorts the same as augusteum
func TestValidatorsSortAugusteum(t *testing.T) {
	...
	// create expected augusteum validators by converting to augusteum then sorting
	expectedVals, err := teststaking.ToTmValidators(valz)
	require.NoError(t, err)
	sort.Sort(tmtypes.ValidatorsByVotingPower(expectedVals))

	// sort in SDK and then convert to augusteum
	sort.Sort(types.ValidatorsByVotingPower(valz))
	actualVals, err := teststaking.ToTmValidators(valz)
	require.NoError(t, err)

	require.Equal(t, expectedVals, actualVals, "sorting in SDK is not the same as sorting in Augusteum")
}
