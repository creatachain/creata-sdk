...
func (sh *Helper) CreateValidator(addr sdk.ValAddress, pk cryptotypes.PubKey, fctaAmount sdk.Int, ok bool) {
	coin := sdk.NewCoin(sh.Denom, fctaAmount)
...
func (sh *Helper) CreateValidatorMsg(addr sdk.ValAddress, pk cryptotypes.PubKey, fctaAmount sdk.Int) *stakingtypes.MsgCreateValidator {
	coin := sdk.NewCoin(sh.Denom, fctaAmount)
...
// Delegate calls handler to delegate ucta for a validator
...
// DelegateWithPower calls handler to delegate ucta for a validator
...
// Undelegate calls handler to unbound some ucta from a validator.
