...
func (k Keeper) HandleEquivocationEvidence(ctx sdk.Context, evidence *types.Equivocation) {
	...
	if _, err := k.slashingKeeper.GetPubkey(ctx, consAddr.Bytes()); err != nil {
		...
		// but this couples the expectations of the app to both Augusteum and
	...
	if validator == nil || validator.IsUnbonded() {
		...
		// Augusteum might break this assumption at some point.
		return
	}
	...
	// We need to retrieve the ucta distribution which signed the block, so we
	...
	// to/by Augusteum. This value is validator.Tokens as sent to Augusteum via
	// MSM, and now received as evidence. The fraction is passed in to separately
