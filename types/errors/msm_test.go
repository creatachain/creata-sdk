...
func TestMSMTestSuite(t *testing.T) {
	suite.Run(t, new(msmTestSuite))
}

func (s *msmTestSuite) SetupSuite() {
	s.T().Parallel()
}

func (s *msmTestSuite) TestMSMnfo() {
	...
	for testName, tc := range cases {
		space, code, log := MSMInfo(tc.err, tc.debug)
...
func (s *msmTestSuite) TestMSMInfoStacktrace() {
	cases := map[string]struct {
		err            error
		debug          bool
		wantStacktrace bool
	...
	const thisTestSrc = "github.com/creatachain/creata-sdk/types/errors.(*msmTestSuite).TestMSMInfoStacktrace"

	for testName, tc := range cases {
		_, _, log := MSMInfo(tc.err, tc.debug)
...
func (s *msmTestSuite) TestMSMInfoHidesStacktrace() {
	err := Wrap(ErrUnauthorized, "wrapped")
	_, _, log := MSMInfo(err, false)
	s.Require().Equal("wrapped: unauthorized", log)
}

func (s *msmTestSuite) TestRedact() {
	cases := map[string]struct {
		err       error
		...
		"pass though custom errors with MSM code": {
			err:       customErr{},
			untouched: true,
		},
		"redact stdlib error": {
	...
	for name, tc := range cases {
		...
		// make sure the MSM code did not change
		s.Require().Equal(msmCode(spec.err), msmCode(redacted), name)

	}
}

func (s *msmTestSuite) TestMSMInfoSerializeErr() {
	var (
	...
	for msg, spec := range specs {
		spec := spec
		_, _, log := MSMInfo(spec.src, spec.debug)
		s.Require().Equal(spec.exp, log, msg)
	}
}

// customErr is a custom implementation of an error that provides an MSMCode
// method.
type customErr struct{}

func (customErr) Codespace() string { return "extern" }

func (customErr) MSMCode() uint32 { return 999 }
