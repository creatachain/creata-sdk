...
func (s *errorsTestSuite) TestMSMError() {
	s.Require().Equal("custom: tx parse error", MSMError(RootCodespace, 2, "custom").Error())
	s.Require().Equal("custom: unknown", MSMError("unknown", 1, "custom").Error())
}
