...
import (
	...
	testdata "github.com/creatachain/creata-sdk/testutil/testdata"
...
func (s *eventsTestSuite) TestToMSMEvents() {
	e := sdk.Events{sdk.NewEvent("transfer", sdk.NewAttribute("sender", "foo"))}
	msmEvents := e.ToMSMEvents()
	s.Require().Len(msmEvents, 1)
	s.Require().Equal(msmEvents[0].Type, e[0].Type)
	s.Require().Equal(msmEvents[0].Attributes, e[0].Attributes)
}
...
func (s *eventsTestSuite) TestEventManagerTypedEvents() {
	...
	msg1, err := sdk.ParseTypedEvent(em.Events().ToMSMEvents()[0])
	s.Require().NoError(err)
	s.Require().Equal(coin.String(), msg1.String())
	s.Require().Equal(reflect.TypeOf(&coin), reflect.TypeOf(msg1))

	msg2, err := sdk.ParseTypedEvent(em.Events().ToMSMEvents()[1])
	s.Require().NoError(err)
...
func (s *eventsTestSuite) TestStringifyEvents() {
	...
	se := sdk.StringifyEvents(e.ToMSMEvents())
