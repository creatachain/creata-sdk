...
import (
	...
	"github.com/creatachain/creata-sdk/store/gaskv"
	stypes "github.com/creatachain/creata-sdk/store/types"
)
...
type Context struct {
	...
	txBytes       []byte
	logger        log.Logger
	voteInfo      []msm.VoteInfo
	gasMeter      GasMeter
	blockGasMeter GasMeter
	checkTx       bool
	recheckTx     bool // if recheckTx == true, then checkTx must also be true
	minGasPrice   DecCoins
	consParams    *msm.ConsensusParams
...
func (c Context) VoteInfos() []msm.VoteInfo   { return c.voteInfo }
...
func (c Context) ConsensusParams() *msm.ConsensusParams {
	return proto.Clone(c.consParams).(*msm.ConsensusParams)
}
...
// WithBlockHeader returns a Context with an updated augusteum block header in UTC time.
...
// WithBlockTime returns a Context with an updated augusteum block header time in UTC time
...
func (c Context) WithVoteInfos(voteInfo []msm.VoteInfo) Context {
	c.voteInfo = voteInfo
	return c
}
...
func (c Context) WithConsensusParams(params *msm.ConsensusParams) Context {
	c.consParams = params
	return c
}
