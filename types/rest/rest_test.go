...
func TestBaseReq_Sanitize(t *testing.T) {
	...
	sanitized := rest.BaseReq{ChainID: "   test",
		Memo:          "memo     ",
		From:          " creata1cq0sxam6x4l0sv9yz3a2vlqhdhvt2k6jtgcse0 ",
		Gas:           " ",
		GasAdjustment: "  0.3",
	}.Sanitize()
...
func TestBaseReq_ValidateBasic(t *testing.T) {
	fromAddr := "creata1cq0sxam6x4l0sv9yz3a2vlqhdhvt2k6jtgcse0"
	tenfctas, err := types.ParseCoinsNormalized("10fcta")
	require.NoError(t, err)
	onefcta, err := types.ParseDecCoins("1.0fcta")
	require.NoError(t, err)

	req1 := rest.NewBaseReq(
		fromAddr, "", "nonempty", "", "", 0, 0, tenfctas, nil, false,
	)
	req2 := rest.NewBaseReq(
		"", "", "nonempty", "", "", 0, 0, tenfctas, nil, false,
	)
	req3 := rest.NewBaseReq(
		fromAddr, "", "", "", "", 0, 0, tenfctas, nil, false,
	)
	req4 := rest.NewBaseReq(
		fromAddr, "", "nonempty", "", "", 0, 0, tenfctas, onefcta, false,
	)
...
func TestProcessPostResponse(t *testing.T) {
	...
	coins := types.NewCoins(types.NewCoin("cta", types.NewInt(100)), types.NewCoin("tree", types.NewInt(125)))
	...
	cdc.RegisterConcrete(&mockAccount{}, "creata-sdk/mockAccount", nil)
