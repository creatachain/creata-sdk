...
import (
	...
	"github.com/creatachain/creata-sdk/tests/mocks"
...
func TestGenesisOnlyAppModule(t *testing.T) {
	...
	goam.BeginBlock(sdk.Context{}, msm.RequestBeginBlock{})
	require.Equal(t, []msm.ValidatorUpdate{}, goam.EndBlock(sdk.Context{}, msm.RequestEndBlock{}))
}
...
func TestManager_InitGenesis(t *testing.T) {
	...
	require.Equal(t, msm.ResponseInitChain{Validators: []msm.ValidatorUpdate(nil)}, mm.InitGenesis(ctx, cdc, genesisData))

	// test panic
	genesisData = map[string]json.RawMessage{
		"module1": json.RawMessage(`{"key": "value"}`),
		"module2": json.RawMessage(`{"key": "value"}`)}
	mockAppModule1.EXPECT().InitGenesis(gomock.Eq(ctx), gomock.Eq(cdc), gomock.Eq(genesisData["module1"])).Times(1).Return([]msm.ValidatorUpdate{{}})
	mockAppModule2.EXPECT().InitGenesis(gomock.Eq(ctx), gomock.Eq(cdc), gomock.Eq(genesisData["module2"])).Times(1).Return([]msm.ValidatorUpdate{{}})
...
func TestManager_BeginBlock(t *testing.T) {
	...
	req := msm.RequestBeginBlock{Hash: []byte("test")}
...
func TestManager_EndBlock(t *testing.T) {
	...
	req := msm.RequestEndBlock{Height: 10}

	mockAppModule1.EXPECT().EndBlock(gomock.Any(), gomock.Eq(req)).Times(1).Return([]msm.ValidatorUpdate{{}})
	mockAppModule2.EXPECT().EndBlock(gomock.Any(), gomock.Eq(req)).Times(1)
	ret := mm.EndBlock(sdk.Context{}, req)
	require.Equal(t, []msm.ValidatorUpdate{{}}, ret.ValidatorUpdates)

	// test panic
	mockAppModule1.EXPECT().EndBlock(gomock.Any(), gomock.Eq(req)).Times(1).Return([]msm.ValidatorUpdate{{}})
	mockAppModule2.EXPECT().EndBlock(gomock.Any(), gomock.Eq(req)).Times(1).Return([]msm.ValidatorUpdate{{}})
