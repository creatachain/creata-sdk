...
// MSMMessageLogs represents a slice of MSMMessageLog.
type MSMMessageLogs []MSMMessageLog

func NewMSMMessageLog(i uint32, log string, events Events) MSMMessageLog {
	return MSMMessageLog{
		MsgIndex: i,
		Log:      log,
		Events:   StringifyEvents(events.ToMSMEvents()),
	}
}

// String implements the fmt.Stringer interface for the MSMMessageLogs type.
func (logs MSMMessageLogs) String() (str string) {
	if logs != nil {
...
// NewResponseResultTx returns a TxResponse given a ResultTx from augusteum
func NewResponseResultTx(res *ctypes.ResultTx, anyTx *codectypes.Any, timestamp string) *TxResponse {
	if res == nil {
		return nil
	}

	parsedLogs, _ := ParseMSMLogs(res.TxResult.Log)
...
// ResultBroadcastTxCommit from augusteum.
...
func newTxResponseCheckTx(res *ctypes.ResultBroadcastTxCommit) *TxResponse {
	...
	parsedLogs, _ := ParseMSMLogs(res.CheckTx.Log)
...
func newTxResponseDeliverTx(res *ctypes.ResultBroadcastTxCommit) *TxResponse {
	...
	parsedLogs, _ := ParseMSMLogs(res.DeliverTx.Log)
...
// NewResponseFormatBroadcastTx returns a TxResponse given a ResultBroadcastTx from augusteum
func NewResponseFormatBroadcastTx(res *ctypes.ResultBroadcastTx) *TxResponse {
	if res == nil {
		return nil
	}

	parsedLogs, _ := ParseMSMLogs(res.Log)
...
// ParseMSMLogs attempts to parse a stringified MSM tx log into a slice of
// MSMMessageLog types. It returns an error upon JSON decoding failure.
func ParseMSMLogs(logs string) (res MSMMessageLogs, err error) {
