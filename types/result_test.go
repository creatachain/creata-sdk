...
func (s *resultTestSuite) TestParseMSMLog() {
	logs := `[{"log":"","msg_index":1,"success":true}]`
	res, err := sdk.ParseMSMLogs(logs)
...
func (s *resultTestSuite) TestMSMMessageLog() {
	cdc := codec.NewLegacyAmino()
	events := sdk.Events{sdk.NewEvent("transfer", sdk.NewAttribute("sender", "foo"))}
	msgLog := sdk.NewMSMMessageLog(0, "", events)
	msgLogs := sdk.MSMMessageLogs{msgLog}
...
func (s *resultTestSuite) TestResponseResultTx() {
	deliverTxResult := msm.ResponseDeliverTx{
		Codespace: "codespace",
		Code:      1,
		Data:      []byte("data"),
		Log:       `[]`,
...
func (s *resultTestSuite) TestResponseFormatBroadcastTxCommit() {
	...
	deliverTxResult := &ctypes.ResultBroadcastTxCommit{
		Height: 10,
		Hash:   bytes.HexBytes([]byte("test")),
		DeliverTx: msm.ResponseDeliverTx{
			Code:      90,
			Data:      nil,
			Log:       `[]`,
			Info:      "info",
