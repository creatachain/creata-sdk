...
// MSMEvents returns all stored Event objects as msm.Event objects.
func (em EventManager) MSMEvents() []msm.Event {
	return em.events.ToMSMEvents()
}
...
func TypedEventToEvent(tev proto.Message) (Event, error) {
	...
	attrs := make([]msm.EventAttribute, 0, len(attrMap))
	for k, v := range attrMap {
		attrs = append(attrs, msm.EventAttribute{
			Key:   []byte(k),
			Value: v,
		})
	}
...
// ParseTypedEvent converts msm.Event back to typed event
func ParseTypedEvent(event msm.Event) (proto.Message, error) {
...
type (
	// Event is a type alias for an MSM Event
	Event msm.Event
...
// ToKVPair converts an Attribute object into a Augusteum key/value pair.
func (a Attribute) ToKVPair() msm.EventAttribute {
	return msm.EventAttribute{Key: toBytes(a.Key), Value: toBytes(a.Value)}
}
...
// ToMSMEvents converts a slice of Event objects to a slice of msm.Event
// objects.
func (e Events) ToMSMEvents() []msm.Event {
	res := make([]msm.Event, len(e))
	for i, ev := range e {
		res[i] = msm.Event{Type: ev.Type, Attributes: ev.Attributes}
	}
...
func StringifyEvent(e msm.Event) StringEvent {
...
// objects.
func StringifyEvents(events []msm.Event) StringEvents {
...
// MarkEventsToIndex returns the set of MSM events, where each event's attribute
// has it's index value marked based on the provided set of events to index.
func MarkEventsToIndex(events []msm.Event, indexSet map[string]struct{}) []msm.Event {
	indexAll := len(indexSet) == 0
	updatedEvents := make([]msm.Event, len(events))

	for i, e := range events {
		updatedEvent := msm.Event{
			Type:       e.Type,
			Attributes: make([]msm.EventAttribute, len(e.Attributes)),
		}

		for j, attr := range e.Attributes {
			_, index := indexSet[fmt.Sprintf("%s.%s", e.Type, attr.Key)]
			updatedAttr := msm.EventAttribute{
				Key:   attr.Key,
				Value: attr.Value,
