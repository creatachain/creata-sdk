...
import (
	...
	"github.com/creatachain/creata-sdk/snapshots/types"
...
// mirroring the MSM interface.
//
// Although the MSM interface (and this manager) passes chunks as byte slices, the internal
// snapshot/restore APIs use IO streams (i.e. chan io.ReadCloser), for two reasons:
//
// 1) In the future, MSM should support streaming. Consider e.g. InitChain during chain
...
// List lists snapshots, mirroring MSM ListSnapshots. It can be concurrent with other operations.
func (m *Manager) List() ([]*types.Snapshot, error) {
	return m.store.List()
}

// LoadChunk loads a chunk into a byte slice, mirroring MSM LoadChunk. It can be called
...
// Restore begins an async snapshot restoration, mirroring MSM OfferSnapshot. Chunks must be fed
...
// RestoreChunk adds a chunk to an active snapshot restoration, mirroring MSM ApplySnapshotChunk.
