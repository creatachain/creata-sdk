...
import (
	...
	tmconfig "github.com/creatachain/augusteum/config"
...
// get cmd to initialize all files for augusteum testnet and application
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a creataapp testnet",
...
Example:
	creatad testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "creatad", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress, "192.168.0.1", "Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...)")
	cmd.Flags().String(flags.FlagChainID, "", "genesis file chain-id, if left blank will be randomly created")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", sdk.DefaultBondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001fcta)")
...
func InitTestnet(
	...
	creataappConfig := srvconfig.DefaultConfig()
	creataappConfig.MinGasPrices = minGasPrices
	creataappConfig.API.Enable = true
	creataappConfig.Telemetry.Enabled = true
	creataappConfig.Telemetry.PrometheusRetentionTime = 60
	creataappConfig.Telemetry.EnableHostnameLabel = false
	creataappConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), creataappConfig)
	}
