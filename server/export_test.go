...
func TestExportCmd_ConsensusParams(t *testing.T) {
	...
	require.Equal(t, creataapp.DefaultConsensusParams.Block.MaxBytes, exportedGenDoc.ConsensusParams.Block.MaxBytes)
	require.Equal(t, creataapp.DefaultConsensusParams.Block.MaxGas, exportedGenDoc.ConsensusParams.Block.MaxGas)

	require.Equal(t, creataapp.DefaultConsensusParams.Evidence.MaxAgeDuration, exportedGenDoc.ConsensusParams.Evidence.MaxAgeDuration)
	require.Equal(t, creataapp.DefaultConsensusParams.Evidence.MaxAgeNumBlocks, exportedGenDoc.ConsensusParams.Evidence.MaxAgeNumBlocks)

	require.Equal(t, creataapp.DefaultConsensusParams.Validator.PubKeyTypes, exportedGenDoc.ConsensusParams.Validator.PubKeyTypes)
}
...
func setupApp(t *testing.T, tempDir string) (*creataapp.CreataApp, context.Context, *tmtypes.GenesisDoc, *cobra.Command) {
	...
	app := creataapp.NewCreataApp(logger, db, nil, true, map[int64]bool{}, tempDir, 0, encCfg, creataapp.EmptyAppOptions{})
	...
	app.InitChain(
		msm.RequestInitChain{
			...
			ConsensusParams: creataapp.DefaultConsensusParams,
	...
	cmd := server.ExportCmd(
		func(_ log.Logger, _ dbm.DB, _ io.Writer, height int64, forZeroHeight bool, jailAllowedAddrs []string, appOptons types.AppOptions) (types.ExportedApp, error) {
			...
			if height != -1 {
				creataApp = creataapp.NewCreataApp(logger, db, nil, false, map[int64]bool{}, "", 0, encCfg, appOptons)

				if err := creataApp.LoadHeight(height); err != nil {
					return types.ExportedApp{}, err
				}
			} else {
				creataApp = creataapp.NewCreataApp(logger, db, nil, true, map[int64]bool{}, "", 0, encCfg, appOptons)
			}
...
func newDefaultGenesisDoc(cdc codec.Marshaler) *tmtypes.GenesisDoc {
	genesisState := creataapp.NewDefaultGenesisState(cdc)
