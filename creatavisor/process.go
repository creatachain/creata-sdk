...
// to happened with "start" but may happened with short-lived commands like `creatad export ...`
func WaitForUpgradeOrExit(cmd *exec.Cmd, scanOut, scanErr *bufio.Scanner) (*UpgradeInfo, error) {
	...
	// if the command exits normally (eg. short command like `creatad version`), just return (nil, nil)
