...
// CheckAugusteumError checks if the error returned from BroadcastTx is a
// Augusteum error that is returned before the tx is submitted due to
// precondition checks that failed. If an Augusteum error is detected, this
...
// a change to Augusteum's RPCError type to allow retrieval or matching against
// a concrete error type.
func CheckAugusteumError(err error, txBytes []byte) *sdk.TxResponse {
	if err == nil {
		return nil
	}
	...
	case strings.Contains(errStr, strings.ToLower(mempool.ErrTxInCache.Error())):
		return &sdk.TxResponse{
			Code:      sdkerrors.ErrTxInMempoolCache.MSMCode(),
	...
	case strings.Contains(errStr, "mempool is full"):
		return &sdk.TxResponse{
			Code:      sdkerrors.ErrMempoolIsFull.MSMCode(),
	...
	case strings.Contains(errStr, "tx too large"):
		return &sdk.TxResponse{
			Code:      sdkerrors.ErrTxTooLarge.MSMCode(),
...
// BroadcastTxCommit broadcasts transaction bytes to a Augusteum node and
...
func (ctx Context) BroadcastTxCommit(txBytes []byte) (*sdk.TxResponse, error) {
	...
	if errRes := CheckAugusteumError(err, txBytes); errRes != nil {
		return errRes, nil
	}
	return sdk.NewResponseFormatBroadcastTxCommit(res), err
}

// BroadcastTxSync broadcasts transaction bytes to a Augusteum node
...
// BroadcastTxAsync broadcasts transaction bytes to a Augusteum node
