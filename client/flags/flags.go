...
func AddQueryFlagsToCmd(cmd *cobra.Command) {
	cmd.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to Augusteum RPC interface for this chain")
...
func AddTxFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 10ucta")
	cmd.Flags().String(FlagGasPrices, "", "Gas prices in decimal format to determine the transaction fee (e.g. 0.1ucta)")
	cmd.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to augusteum rpc interface for this chain")
