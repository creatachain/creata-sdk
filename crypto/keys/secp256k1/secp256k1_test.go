...
import (
	...
	tmsecp256k1 "github.com/creatachain/augusteum/crypto/secp256k1"
...
func TestGenPrivKeyFromSecret(t *testing.T) {
	...
	tests := []struct {
		...
		{"another seed used in creata tests #1", []byte{0}},
		{"another seed used in creata tests #2", []byte("mySecret")},
		{"another seed used in creata tests #3", []byte("")},
	}
...
func TestMarshalAmino_BackwardsCompatibility(t *testing.T) {
	aminoCdc := codec.NewLegacyAmino()
	// Create Augusteum keys.
	tmPrivKey := tmsecp256k1.GenPrivKey()
	tmPubKey := tmPrivKey.PubKey()
	// Create our own keys, with the same private key as Augusteum's.
