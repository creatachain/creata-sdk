...
import (
	...
	sdkmaps "github.com/creatachain/creata-sdk/store/internal/maps"
...
func TestMultiStoreQuery(t *testing.T) {
	...
	// Test bad path.
	query := msm.RequestQuery{Path: "/key", Data: k, Height: ver}
	qres := multi.Query(query)
	require.EqualValues(t, sdkerrors.ErrUnknownRequest.MSMCode(), qres.Code)
...
func benchmarkMultistoreSnapshot(b *testing.B, stores uint8, storeKeys uint64) {
	b.Skip("Noisy with slow setup time,")
