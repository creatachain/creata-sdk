...
func TestCreateMembership(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			root := CalcRoot(data)
			err = proof.GetExist().Verify(AugusteumSpec, root, []byte(key), val)
			if err != nil {
				t.Fatalf("Verifying Proof: %+v", err)
			}

			valid := ics23.VerifyMembership(AugusteumSpec, root, proof, []byte(key), val)
			if !valid {
...
func TestCreateNonMembership(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			root := CalcRoot(data)
			err = proof.GetNonexist().Verify(AugusteumSpec, root, []byte(key))
			if err != nil {
				t.Fatalf("Verifying Proof: %+v", err)
			}

			valid := ics23.VerifyNonMembership(AugusteumSpec, root, proof, []byte(key))
			if !valid {
