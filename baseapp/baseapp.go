...
// BaseApp reflects the MSM application implementation.
type BaseApp struct { // nolint: maligned
	// initialized on creation
	logger            log.Logger
	name              string               // application name from msm.Info
	...
	voteInfos []msm.VoteInfo

	// paramStore is used to query for MSM consensus parameters from an
	...
	// from Augusteum. It is used as part of the process of determining the
	// ResponseCommit.RetainHeight value during MSM Commit. A value of 0 indicates
	// that no blocks should be pruned.
	//
	// Note: Augusteum block pruning is dependant on this parameter in conunction
	...
	// trace set will return full stack traces for errors in MSM Log field
	trace bool

	// indexEvents defines the set of events in the form {eventType}.{attributeKey},
	// which informs Augusteum what to index. If empty, all events will be indexed.
...
func (app *BaseApp) GetConsensusParams(ctx sdk.Context) *msm.ConsensusParams {
	if app.paramStore == nil {
		return nil
	}

	cp := new(msm.ConsensusParams)

	if app.paramStore.Has(ctx, ParamStoreKeyBlockParams) {
		var bp msm.BlockParams
...
func (app *BaseApp) StoreConsensusParams(ctx sdk.Context, cp *msm.ConsensusParams) {
	if app.paramStore == nil {
...
func (app *BaseApp) validateHeight(req msm.RequestBeginBlock) error {
...
func (app *BaseApp) runTx(mode runTxMode, txBytes []byte) (gInfo sdk.GasInfo, result *sdk.Result, err error) {
	...
	if err == nil && mode == runTxModeDeliver {
		...
		if len(events) > 0 {
			...
			result.Events = append(events.ToMSMEvents(), result.Events...)
		}
	}
...
func (app *BaseApp) runMsgs(ctx sdk.Context, msgs []sdk.Msg, mode runTxMode) (*sdk.Result, error) {
	msgLogs := make(sdk.MSMMessageLogs, 0, len(msgs))
	...
	for i, msg := range msgs {
		...
		msgLogs = append(msgLogs, sdk.NewMSMMessageLog(uint32(i), msgResult.Log, msgEvents))
	}
