...
// GRPCQueryRouter routes MSM Query requests to GRPC handlers
type GRPCQueryRouter struct {
	...
	// decode the MSM response bytes using the cached return type.
...
// GRPCQueryHandler defines a function type which handles MSM Query requests
// using gRPC
type GRPCQueryHandler = func(ctx sdk.Context, req msm.RequestQuery) (msm.ResponseQuery, error)
...
func (qrt *GRPCQueryRouter) RegisterService(sd *grpc.ServiceDesc, handler interface{}) {
	...
	for _, method := range sd.Methods {
		...
		qrt.routes[fqName] = func(ctx sdk.Context, req msm.RequestQuery) (msm.ResponseQuery, error) {
			// call the method handler from the service description with the handler object,
			// a wrapped sdk.Context with proto-unmarshaled data from the MSM request data
