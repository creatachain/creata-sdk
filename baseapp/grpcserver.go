...
func (app *BaseApp) RegisterGRPCServer(clientCtx client.Context, server gogogrpc.Server) {
	...
	// the query through the `clientCtx`, which itself queries Augusteum.
	interceptor := func(grpcCtx context.Context, req interface{}, info *grpc.UnaryServerInfo, _ grpc.UnaryHandler) (interface{}, error) {
		...
		// 1. either we're broadcasting a Tx, in which case we call Augusteum's broadcast endpoint directly,
		...
		msmRes, outMd, err := client.RunGRPCQuery(clientCtx, grpcCtx, info.FullMethod, req, inMd)
		if err != nil {
			return nil, err
		}

		// We need to know the return type of the grpc method for
		// unmarshalling msmRes.Value.
		//
		...
		err = protoCodec.Unmarshal(msmRes.Value, res)
		if err != nil {
			return nil, err
		}
