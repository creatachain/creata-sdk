...
import (
	...
	snapshottypes "github.com/creatachain/creata-sdk/snapshots/types"
...
func registerTestCodec(cdc *codec.LegacyAmino) {
	...
	// register test types
	cdc.RegisterConcrete(&txTest{}, "creata-sdk/baseapp/txTest", nil)
	cdc.RegisterConcrete(&msgCounter{}, "creata-sdk/baseapp/msgCounter", nil)
	cdc.RegisterConcrete(&msgCounter2{}, "creata-sdk/baseapp/msgCounter2", nil)
	cdc.RegisterConcrete(&msgKeyValue{}, "creata-sdk/baseapp/msgKeyValue", nil)
	cdc.RegisterConcrete(&msgNoRoute{}, "creata-sdk/baseapp/msgNoRoute", nil)
}
...
func setupBaseAppWithSnapshots(t *testing.T, blocks uint, blockTxs int, options ...func(*BaseApp)) (*BaseApp, func()) {
	...
	app.InitChain(msm.RequestInitChain{})

	r := rand.New(rand.NewSource(3920758213583))
	keyCounter := 0
	for height := int64(1); height <= int64(blocks); height++ {
		app.BeginBlock(msm.RequestBeginBlock{Header: tmproto.Header{Height: height}})
		for txNum := 0; txNum < blockTxs; txNum++ {
			...
			require.NoError(t, err)
			resp := app.DeliverTx(msm.RequestDeliverTx{Tx: txBytes})
			require.True(t, resp.IsOK(), "%v", resp.String())
		}
		app.EndBlock(msm.RequestEndBlock{Height: height})
		app.Commit()
...
func TestSetLoader(t *testing.T) {
	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// "execute" one block
			app.BeginBlock(msm.RequestBeginBlock{Header: tmproto.Header{Height: 2}})
			res := app.Commit()
...
func TestAppVersionSetterGetter(t *testing.T) {
	...
	res := app.Query(msm.RequestQuery{Path: "app/version"})
...
func TestLoadVersionPruning(t *testing.T) {
	...
	for i := int64(1); i <= 7; i++ {
		app.BeginBlock(msm.RequestBeginBlock{Header: tmproto.Header{Height: i}})
		res := app.Commit()
...
func TestSetMinGasPrices(t *testing.T) {
	minGasPrices := sdk.DecCoins{sdk.NewInt64DecCoin("ucta", 5000)}
...
func TestInitChainer(t *testing.T) {
	...
	var initChainer sdk.InitChainer = func(ctx sdk.Context, req msm.RequestInitChain) msm.ResponseInitChain {
	...
	initChainRes := app.InitChain(msm.RequestInitChain{AppStateBytes: []byte("{}"), ChainId: "test-chain-id"}) // must have valid JSON genesis file, even if empty
...
func TestBeginBlock_WithInitialHeight(t *testing.T) {
	...
	require.PanicsWithError(t, "invalid height: 4; expected: 3", func() {
		app.BeginBlock(msm.RequestBeginBlock{
...
func handlerMsgCounter(t *testing.T, capKey sdk.StoreKey, deliverKey []byte) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		res.Events = ctx.EventManager().Events().ToMSMEvents()
		return res, nil
	}
}
...
func TestCheckTx(t *testing.T) {
	...
	for i := int64(0); i < nTxs; i++ {
		...
		txBytes, err := codec.MarshalBinaryBare(tx)
		require.NoError(t, err)
		r := app.CheckTx(msm.RequestCheckTx{Tx: txBytes})
...
func TestDeliverTx(t *testing.T) {
	...
	for blockN := 0; blockN < nBlocks; blockN++ {
		...
		for i := 0; i < txPerHeight; i++ {
			...
			res := app.DeliverTx(msm.RequestDeliverTx{Tx: txBytes})
			require.True(t, res.IsOK(), fmt.Sprintf("%v", res))
			events := res.GetEvents()
			require.Len(t, events, 3, "should contain ante handler, message type and counter events respectively")
			require.Equal(t, sdk.MarkEventsToIndex(counterEvent("ante_handler", counter).ToMSMEvents(), map[string]struct{}{})[0], events[0], "ante handler event")
			require.Equal(t, sdk.MarkEventsToIndex(counterEvent(sdk.EventTypeMessage, counter).ToMSMEvents(), map[string]struct{}{})[0], events[2], "msg handler update counter event")
		}
...
func TestRunInvalidTransaction(t *testing.T) {
	...
	// transaction with no messages
	{
		...
		space, code, _ := sdkerrors.MSMInfo(err, false)
		require.EqualValues(t, sdkerrors.ErrInvalidRequest.Codespace(), space, err)
		require.EqualValues(t, sdkerrors.ErrInvalidRequest.MSMCode(), code, err)
	}
	...
	// transaction where ValidateBasic fails
	{
		...
		for _, testCase := range testCases {
			...
			if testCase.fail {
				...
				require.EqualValues(t, sdkerrors.ErrInvalidSequence.MSMCode(), code, err)
			} else {
				require.NotNil(t, result)
			}
		}
	...
	// transaction with no known route
	{
		...
		require.EqualValues(t, sdkerrors.ErrUnknownRequest.MSMCode(), code, err)
	...
	// Transaction with an unregistered message
	{
		...
		newCdc.RegisterConcrete(&msgNoDecode{}, "creata-sdk/baseapp/msgNoDecode", nil)

		txBytes, err := newCdc.MarshalBinaryBare(tx)
		require.NoError(t, err)

		res := app.DeliverTx(msm.RequestDeliverTx{Tx: txBytes})
		require.EqualValues(t, sdkerrors.ErrTxDecode.MSMCode(), res.Code)
...
func TestTxGasLimits(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if !tc.fail {
			...
			require.EqualValues(t, sdkerrors.ErrOutOfGas.MSMCode(), code, err)
		}
	}
}
...
func TestGRPCQuery(t *testing.T) {
	...
	require.Equal(t, msm.CodeTypeOK, resQuery.Code, resQuery)
...
func TestP2PQuery(t *testing.T) {
	addrPeerFilterOpt := func(bapp *BaseApp) {
		bapp.SetAddrPeerFilter(func(addrport string) msm.ResponseQuery {
			require.Equal(t, "1.1.1.1:8000", addrport)
			return msm.ResponseQuery{Code: uint32(3)}
		})
	}

	idPeerFilterOpt := func(bapp *BaseApp) {
		bapp.SetIDPeerFilter(func(id string) msm.ResponseQuery {
			require.Equal(t, "testid", id)
			return msm.ResponseQuery{Code: uint32(4)}
		})
	}

	app := setupBaseApp(t, addrPeerFilterOpt, idPeerFilterOpt)

	addrQuery := msm.RequestQuery{
		Path: "/p2p/filter/addr/1.1.1.1:8000",
	}
	res := app.Query(addrQuery)
	require.Equal(t, uint32(3), res.Code)

	idQuery := msm.RequestQuery{
...
func TestGetMaximumBlockGas(t *testing.T) {
	...
	app.StoreConsensusParams(ctx, &msm.ConsensusParams{Block: &msm.BlockParams{MaxGas: 0}})
	require.Equal(t, uint64(0), app.getMaximumBlockGas(ctx))

	app.StoreConsensusParams(ctx, &msm.ConsensusParams{Block: &msm.BlockParams{MaxGas: -1}})
	require.Equal(t, uint64(0), app.getMaximumBlockGas(ctx))

	app.StoreConsensusParams(ctx, &msm.ConsensusParams{Block: &msm.BlockParams{MaxGas: 5000000}})
...
func TestListSnapshots(t *testing.T) {
	...
	resp := app.ListSnapshots(msm.RequestListSnapshots{})
	...
	assert.Equal(t, msm.ResponseListSnapshots{Snapshots: []*msm.Snapshot{
...
func TestLoadSnapshotChunk(t *testing.T) {
	...
	for name, tc := range testcases {
		tc := tc
		t.Run(name, func(t *testing.T) {
			resp := app.LoadSnapshotChunk(msm.RequestLoadSnapshotChunk{
				Height: tc.height,
				Format: tc.format,
				Chunk:  tc.chunk,
			})
			if tc.expectEmpty {
				assert.Equal(t, msm.ResponseLoadSnapshotChunk{}, resp)
				return
			}
...
func TestOfferSnapshot_Errors(t *testing.T) {
	...
	testcases := map[string]struct {
		snapshot *msm.Snapshot
		result   msm.ResponseOfferSnapshot_Result
	}{
		"nil snapshot": {nil, msm.ResponseOfferSnapshot_REJECT},
		"invalid format": {&msm.Snapshot{
			Height: 1, Format: 9, Chunks: 3, Hash: hash, Metadata: metadata,
		}, msm.ResponseOfferSnapshot_REJECT_FORMAT},
		"incorrect chunk count": {&msm.Snapshot{
			Height: 1, Format: 1, Chunks: 2, Hash: hash, Metadata: metadata,
		}, msm.ResponseOfferSnapshot_REJECT},
		"no chunks": {&msm.Snapshot{
			Height: 1, Format: 1, Chunks: 0, Hash: hash, Metadata: metadata,
		}, msm.ResponseOfferSnapshot_REJECT},
		"invalid metadata serialization": {&msm.Snapshot{
	...
	for name, tc := range testcases {
		tc := tc
		t.Run(name, func(t *testing.T) {
			resp := app.OfferSnapshot(msm.RequestOfferSnapshot{Snapshot: tc.snapshot})
			assert.Equal(t, tc.result, resp.Result)
		})
	}

	// Offering a snapshot after one has been accepted should error
	resp := app.OfferSnapshot(msm.RequestOfferSnapshot{Snapshot: &msm.Snapshot{
		Height:   1,
	...
	require.Equal(t, msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ACCEPT}, resp)
	...
	require.Equal(t, msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ABORT}, resp)
}
...
func TestApplySnapshotChunk(t *testing.T) {
	...
	respList := source.ListSnapshots(msm.RequestListSnapshots{})
	...
	respOffer := target.OfferSnapshot(msm.RequestOfferSnapshot{Snapshot: snapshot})
	require.Equal(t, msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ACCEPT}, respOffer)

	// We should be able to pass an invalid chunk and get a verify failure, before reapplying it.
	respApply := target.ApplySnapshotChunk(msm.RequestApplySnapshotChunk{
		Index:  0,
		Chunk:  []byte{9},
		Sender: "sender",
	})
	require.Equal(t, msm.ResponseApplySnapshotChunk{
		Result:        msm.ResponseApplySnapshotChunk_RETRY,
	...
	for index := uint32(0); index < snapshot.Chunks; index++ {
		respChunk := source.LoadSnapshotChunk(msm.RequestLoadSnapshotChunk{
		...
		require.Equal(t, msm.ResponseApplySnapshotChunk{
			Result: msm.ResponseApplySnapshotChunk_ACCEPT,
		}, respApply)
	}
...
func TestBaseApp_EndBlock(t *testing.T) {
	...
	app.SetEndBlocker(func(ctx sdk.Context, req msm.RequestEndBlock) msm.ResponseEndBlock {
	...
	res := app.EndBlock(msm.RequestEndBlock{})
