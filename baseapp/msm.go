...
// InitChain implements the MSM interface. It runs the initialization logic
// directly on the CommitMultiStore.
func (app *BaseApp) InitChain(req msm.RequestInitChain) (res msm.ResponseInitChain) {
	...
	if len(req.Validators) > 0 {
		...
		sort.Sort(msm.ValidatorUpdates(req.Validators))
		sort.Sort(msm.ValidatorUpdates(res.Validators))
...
// Info implements the MSM interface.
func (app *BaseApp) Info(req msm.RequestInfo) msm.ResponseInfo {
...
// SetOption implements the MSM interface.
func (app *BaseApp) SetOption(req msm.RequestSetOption) (res msm.ResponseSetOption) {

	return
}

// FilterPeerByAddrPort filters peers by address/port.
func (app *BaseApp) FilterPeerByAddrPort(info string) msm.ResponseQuery {
...
func (app *BaseApp) FilterPeerByID(info string) msm.ResponseQuery {
...
// BeginBlock implements the MSM application interface.
func (app *BaseApp) BeginBlock(req msm.RequestBeginBlock) (res msm.ResponseBeginBlock) {
	defer telemetry.MeasureSince(time.Now(), "msm", "begin_block")
...
// EndBlock implements the MSM interface.
func (app *BaseApp) EndBlock(req msm.RequestEndBlock) (res msm.ResponseEndBlock) {
	defer telemetry.MeasureSince(time.Now(), "msm", "end_block")
...
// CheckTx implements the MSM interface and executes a tx in CheckTx mode. In
// CheckTx mode, messages are not executed. This means messages are only validated
// and only the AnteHandler is executed. State is persisted to the BaseApp's
// internal CheckTx state if the AnteHandler passes. Otherwise, the ResponseCheckTx
// will contain releveant error information. Regardless of tx execution outcome,
// the ResponseCheckTx will contain relevant gas execution context.
func (app *BaseApp) CheckTx(req msm.RequestCheckTx) msm.ResponseCheckTx {
	defer telemetry.MeasureSince(time.Now(), "msm", "check_tx")

	var mode runTxMode

	switch {
	case req.Type == msm.CheckTxType_New:
		mode = runTxModeCheck

	case req.Type == msm.CheckTxType_Recheck:
...
// DeliverTx implements the MSM interface and executes a tx in DeliverTx mode.
// State only gets persisted if all messages are valid and get executed successfully.
// Otherwise, the ResponseDeliverTx will contain releveant error information.
// Regardless of tx execution outcome, the ResponseDeliverTx will contain relevant
// gas execution context.
func (app *BaseApp) DeliverTx(req msm.RequestDeliverTx) msm.ResponseDeliverTx {
	defer telemetry.MeasureSince(time.Now(), "msm", "deliver_tx")
...
// Commit implements the MSM interface. It will commit all state that exists in
// the deliver state's multi-store and includes the resulting commit ID in the
// returned msm.ResponseCommit. Commit will set the check state based on the
// latest header and reset the deliver state. Also, if a non-zero halt height is
// defined in config, Commit will execute a deferred function call to check
// against that height and gracefully halt if it matches the latest committed
// height.
func (app *BaseApp) Commit() (res msm.ResponseCommit) {
	defer telemetry.MeasureSince(time.Now(), "msm", "commit")
	...
	//
	// NOTE: This is safe because Augusteum holds a lock on the mempool for
	...
	if halt {
		// Halt the binary and allow Augusteum to receive the ResponseCommit
...
// Query implements the MSM interface. It delegates to CommitMultiStore if it
// implements Queryable.
func (app *BaseApp) Query(req msm.RequestQuery) (res msm.ResponseQuery) {
	defer telemetry.MeasureSince(time.Now(), "msm", "query")
...
// ListSnapshots implements the MSM interface. It delegates to app.snapshotManager if set.
func (app *BaseApp) ListSnapshots(req msm.RequestListSnapshots) msm.ResponseListSnapshots {
	resp := msm.ResponseListSnapshots{Snapshots: []*msm.Snapshot{}}
	if app.snapshotManager == nil {
		return resp
	}
	...
	for _, snapshot := range snapshots {
		msmSnapshot, err := snapshot.ToMSM()
		if err != nil {
			app.logger.Error("failed to list snapshots", "err", err)
			return resp
		}
		resp.Snapshots = append(resp.Snapshots, &msmSnapshot)
	}

	return resp
}

// LoadSnapshotChunk implements the MSM interface. It delegates to app.snapshotManager if set.
func (app *BaseApp) LoadSnapshotChunk(req msm.RequestLoadSnapshotChunk) msm.ResponseLoadSnapshotChunk {
	if app.snapshotManager == nil {
		return msm.ResponseLoadSnapshotChunk{}
	}
...
// OfferSnapshot implements the MSM interface. It delegates to app.snapshotManager if set.
func (app *BaseApp) OfferSnapshot(req msm.RequestOfferSnapshot) msm.ResponseOfferSnapshot {
	if app.snapshotManager == nil {
		app.logger.Error("snapshot manager not configured")
		return msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ABORT}
	}

	if req.Snapshot == nil {
		app.logger.Error("received nil snapshot")
		return msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT}
	}

	snapshot, err := snapshottypes.SnapshotFromMSM(req.Snapshot)
	if err != nil {
	...
	case errors.Is(err, snapshottypes.ErrUnknownFormat):
		return msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT_FORMAT}
	...
	default:
		...
		// so we ask Augusteum to abort all snapshot restoration.
		return msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ABORT}
	}
}

// ApplySnapshotChunk implements the MSM interface. It delegates to app.snapshotManager if set.
func (app *BaseApp) ApplySnapshotChunk(req msm.RequestApplySnapshotChunk) msm.ResponseApplySnapshotChunk {
	if app.snapshotManager == nil {
		app.logger.Error("snapshot manager not configured")
		return msm.ResponseApplySnapshotChunk{Result: msm.ResponseApplySnapshotChunk_ABORT}
	}
	...
	case errors.Is(err, snapshottypes.ErrChunkHashMismatch):
		...
		return msm.ResponseApplySnapshotChunk{
...
func (app *BaseApp) handleQueryGRPC(handler GRPCQueryHandler, req msm.RequestQuery) msm.ResponseQuery {
...
// are pruned from Augusteum. Given a commitment height and a non-zero local
...
func handleQueryApp(app *BaseApp, path []string, req msm.RequestQuery) msm.ResponseQuery {
	if len(path) >= 2 {
		switch path[1] {
		case "simulate":
			txBytes := req.Data
...
func handleQueryStore(app *BaseApp, path []string, req msm.RequestQuery) msm.ResponseQuery {
...
func handleQueryP2P(app *BaseApp, path []string) msm.ResponseQuery {
...
func handleQueryCustom(app *BaseApp, path []string, req msm.RequestQuery) msm.ResponseQuery {
